...
- [ ] 之前有调整过，要求现在的cmake install能够把整个项目安装出来，然后现在dll被install出来了，但是exe没有被安装出来，我需要你至少把application install出来，可选地把test install出来，然后把依赖的其他dll、资源 (观察现在的configure.py，可以修改利用它) 也要放到install的正确目的地去。并且让run_all_tests在那个install的目的地也成功。现在本目录下有个RuzinoInstall，你需要搞到../RuzinoInstall去做测试。
