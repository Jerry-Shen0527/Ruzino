 ...
 *
 * OptiX public API Reference - Device side implementation for transformation
 * helper functions.
 */
...
static __forceinline__ __device__ T optixLoadReadOnlyAlign16(const T* ptr)
{
    ...
    // If T does not enforce 16B alignment, v may not be 16B aligned and storing
    // the loaded data from ptr fails
    __align__(16) T v;
    for (int ofs = 0; ofs < sizeof(T); ofs += 16)
        *(uint4*)((char*)&v + ofs) =
            optixLdg((unsigned long long)((char*)ptr + ofs));
    return v;
}

// Multiplies the row vector vec with the 3x4 matrix with rows m0, m1, and m2
static __forceinline__ __device__ float4 optixMultiplyRowMatrix(
    const float4 vec,
    const float4 m0,
    const float4 m1,
    const float4 m2)
...
// Converts the SRT transformation srt into a 3x4 matrix with rows m0, m1, and
// m2
static __forceinline__ __device__ void optixGetMatrixFromSrt(
    float4& m0,
    float4& m1,
    float4& m2,
...
{
    const float det3 = m0.x * (m1.y * m2.z - m1.z * m2.y) -
...
static __forceinline__ __device__ void optixLoadInterpolatedMatrixKey(
    float4& m0,
    float4& m1,
    float4& m2,
    const float4* matrix,
    const float t1)
{
    ...
    if (t1 > 0.0f) {
        ...
        m0 = optixAddFloat4(
            optixMulFloat4(m0, t0),
            optixMulFloat4(optixLoadReadOnlyAlign16(&matrix[3]), t1));
        m1 = optixAddFloat4(
            optixMulFloat4(m1, t0),
            optixMulFloat4(optixLoadReadOnlyAlign16(&matrix[4]), t1));
        m2 = optixAddFloat4(
            optixMulFloat4(m2, t0),
            optixMulFloat4(optixLoadReadOnlyAlign16(&matrix[5]), t1));
    }
}

static __forceinline__ __device__ void optixLoadInterpolatedSrtKey(
    float4& srt0,
    float4& srt1,
    float4& srt2,
    float4& srt3,
    ...
    const float t1)
{
    ...
    if (t1 > 0.0f) {
        const float t0 = 1.0f - t1;
        srt0 = optixAddFloat4(
            optixMulFloat4(srt0, t0),
            optixMulFloat4(optixLoadReadOnlyAlign16(&srt[4]), t1));
        srt1 = optixAddFloat4(
            optixMulFloat4(srt1, t0),
            optixMulFloat4(optixLoadReadOnlyAlign16(&srt[5]), t1));
        srt2 = optixAddFloat4(
            optixMulFloat4(srt2, t0),
            optixMulFloat4(optixLoadReadOnlyAlign16(&srt[6]), t1));
        srt3 = optixAddFloat4(
            optixMulFloat4(srt3, t0),
            optixMulFloat4(optixLoadReadOnlyAlign16(&srt[7]), t1));

        float inv_length = 1.f / sqrt(
                                     srt2.y * srt2.y + srt2.z * srt2.z +
                                     srt2.w * srt2.w + srt3.x * srt3.x);
        srt2.y *= inv_length;
        srt2.z *= inv_length;
        srt2.w *= inv_length;
        srt3.x *= inv_length;
...
static __forceinline__ __device__ void optixResolveMotionKey(
    float& localt,
    int& key,
    ...
    const float globalt)
{
    ...
    // No need to check the motion flags. If data originates from a valid
    // transform list handle, then globalt is in range, or vanish flags are not
    // set.
    ...
    float time =
        max(0.f,
            min(numIntervals,
                numIntervals *
                    __fdividef(globalt - timeBegin, timeEnd - timeBegin)));
...
// Returns the interpolated transformation matrix for a particular matrix motion
// transformation and point in time.
static __forceinline__ __device__ void optixGetInterpolatedTransformation(
    float4& trf0,
    float4& trf1,
    float4& trf2,
    ...
    const float time)
{
    ...
    optixResolveMotionKey(
        keyTime,
        key,
        optixLoadReadOnlyAlign16(transformData).motionOptions,
        time);

    // Get pointer to left key
    const float4* transform =
        (const float4*)(&transformData->transform[key][0]);

    // Load and interpolate matrix keys
    optixLoadInterpolatedMatrixKey(trf0, trf1, trf2, transform, keyTime);
}

// Returns the interpolated transformation matrix for a particular SRT motion
// transformation and point in time.
static __forceinline__ __device__ void optixGetInterpolatedTransformation(
    float4& trf0,
    float4& trf1,
    ...
    const float time)
{
    ...
    // Get pointer to left key
    const float4* dataPtr =
        reinterpret_cast<const float4*>(&transformData->srtData[key]);

    // Load and interpolated SRT keys
    float4 data[4];
    optixLoadInterpolatedSrtKey(
        data[0], data[1], data[2], data[3], dataPtr, keyTime);

    OptixSRTData srt = { data[0].x, data[0].y, data[0].z, data[0].w,
...
// Returns the interpolated transformation matrix for a particular traversable
// handle and point in time.
static __forceinline__ __device__ void
    ...
    const bool objectToWorld)
{
    ...
    if (type == OPTIX_TRANSFORM_TYPE_MATRIX_MOTION_TRANSFORM ||
        ...
        type == OPTIX_TRANSFORM_TYPE_INSTANCE ||
        type == OPTIX_TRANSFORM_TYPE_STATIC_TRANSFORM) {
        const float4* transform;
        ...
        if (type == OPTIX_TRANSFORM_TYPE_INSTANCE) {
            ...
            transform =
                (const float4*)((objectToWorld) ? traversable->transform
...
// transform stack and current ray time.
static __forceinline__ __device__ void
...
#pragma unroll 1
    for (int i = size - 1; i >= 0; --i) {
        ...
        float4 trf0, trf1, trf2;
        optixGetInterpolatedTransformationFromHandle(
            trf0, trf1, trf2, handle, time, /*objectToWorld*/ true);
...
static __forceinline__ __device__ float3 optixTransformPoint(
    const float4& m0,
    const float4& m1,
    const float4& m2,
    const float3& p)
...
// Multiplies the 3x3 linear submatrix of the 3x4 matrix with rows m0, m1, m2
// with the vector v.
static __forceinline__ __device__ float3 optixTransformVector(
    const float4& m0,
    const float4& m1,
    const float4& m2,
    const float3& v)
...
// Multiplies the transpose of the 3x3 linear submatrix of the 3x4 matrix with
// rows m0, m1, m2 with the normal n. Note that the given matrix is supposed to
// be the inverse of the actual transformation matrix.
static __forceinline__ __device__ float3 optixTransformNormal(
    const float4& m0,
    const float4& m1,
    const float4& m2,
    const float3& n)
