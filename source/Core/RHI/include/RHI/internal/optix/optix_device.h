...
/// Similar to optixTrace, but does not invoke closesthit or miss. Instead, it
/// overwrites the current outgoing hit object with the results of traversing
/// the ray. The outgoing hit object may be invoked at some later point with
/// optixInvoke. The outgoing hit object can also be queried through various
/// functions such as optixHitObjectIsHit or optixHitObjectGetAttribute_0.
///
/// \param[in] handle
/// \param[in] rayOrigin
/// \param[in] rayDirection
...
/// Reorder the current thread using the current outgoing hit object and the
/// coherence hint bits provided.  Note that the coherence hint will take away
/// some of the bits used in the hit object for sorting, so care should be made
/// to reduce the number of hint bits as much as possible. Nop hit objects can
/// use more coherence hint bits. Bits are taken from the lowest significant bit
/// range. The maximum value of numCoherenceHintBitsFromLSB is implementation
...
/// Reorder the current thread using the hit object only, ie without further
/// coherence hints.
///
/// Available in RG
static __forceinline__ __device__ void optixReorder();

/// Invokes closesthit, miss or nop based on the current outgoing hit object.
/// After execution the current outgoing hit object will be set to nop. An
/// implied nop hit object is always assumed to exist even if there are no calls
/// to optixTraverse, optixMakeHitObject, optixMakeMissHitObject, or
/// optixMakeNopHitObject.
///
/// \param[in,out] payload       up to 32 unsigned int values that hold the
/// payload
///
/// Available in RG, CH, MS, CC
template<typename... Payload>
...
/// Constructs an outgoing hit object from the hit information provided. This
/// hit object will now become the current outgoing hit object and will
...
/// Constructs an outgoing hit object from the hit information provided. This
/// hit object will now become the current outgoing hit object and will
/// overwrite the current outgoing hit object. This method includes the ability
/// to specify arbitrary numbers of OptixTraversableHandle pointers for scenes
/// with 0 to OPTIX_DEVICE_PROPERTY_LIMIT_MAX_TRAVERSABLE_GRAPH_DEPTH levels of
/// transforms.
///
/// \param[in] handle
/// \param[in] rayOrigin
...
/// Constructs an outgoing hit object from the hit information provided. The SBT
/// record index is explicitly specified. This hit object will now become the
/// current outgoing hit object and will overwrite the current outgoing hit
/// object.
///
/// \param[in] handle
/// \param[in] rayOrigin
...
/// Constructs an outgoing hit object from the miss information provided. The
/// SBT record index is explicitly specified as an argument. This hit object
/// will now become the current outgoing hit object and will overwrite the
/// current outgoing hit object.
///
/// \param[in] missSBTIndex   specifies the miss program invoked on a miss
/// \param[in] rayOrigin
/// \param[in] rayDirection
...
/// Constructs an outgoing hit object that when invoked does nothing (neither
/// the miss nor the closest hit shader will be invoked). This hit object will
/// now become the current outgoing hit object and will overwrite the current
/// outgoing hit object. Accessors such as optixHitObjectGetInstanceId will
/// return 0 or 0 filled structs. Only optixHitObjectGetIsNop() will return a
/// non-zero result.
///
/// Available in RG, CH, MS, CC
...
/// Returns true if the current outgoing hit object contains neither a hit nor
/// miss. If executed with optixInvoke, no operation will result. An implied nop
/// hit object is always assumed to exist even if there are no calls such as
/// optixTraverse to explicitly create one.
///
/// Available in RG, CH, MS, CC, DC
static __forceinline__ __device__ bool optixHitObjectIsNop();

/// Returns the SBT record index associated with the hit or miss program for the
/// current outgoing hit object.
///
...
/// Writes the 32-bit payload at the given slot index. There are up to 32
/// attributes available. The number of attributes is configured with
/// OptixPipelineCompileOptions::numPayloadValues or with OptixPayloadType
/// parameters set in OptixModuleCompileOptions.
///
...
/// Returns the 32-bit payload at the given slot index. There are up to 32
/// attributes available. The number of attributes is configured with
/// OptixPipelineCompileOptions::numPayloadValues or with OptixPayloadType
/// parameters set in OptixModuleCompileOptions.
///
...
///
/// The supported types are specified as a bitwise combination of payload types.
/// (See OptixPayloadTypeID) May only be called once per program.
///
...
///
/// May be more expensive to call in IS and AH than their object space
/// counterparts, so effort should be made to use the object space ray in those
/// programs.
///
/// Available in IS, AH, CH, MS
...
/// Returns the current object space ray origin based on the current transform
/// stack.
///
/// Available in IS and AH
static __forceinline__ __device__ float3 optixGetObjectRayOrigin();

/// Returns the current object space ray direction based on the current
/// transform stack.
///
/// Available in IS and AH
...
/// In IS and CH returns the current smallest reported hitT or the tmax passed
/// into optixTrace if no hit has been reported
///
...
///
/// To obtain instance traversables by index, the IAS must be built using the
/// flag OPTIX_BUILD_FLAG_ALLOW_RANDOM_INSTANCE_ACCESS.
///
...
/// Return the object space triangle vertex positions of a given triangle in a
...
///
/// If motion is disabled via OptixPipelineCompileOptions::usesMotionBlur, or
/// the GAS does not contain motion, the time parameter is ignored.
///
/// Available in all OptiX program types
...
/// Return the object space micro triangle vertex positions of the current hit.
/// The current hit must be a displacement micromap triangle hit.
///
/// Available in all OptiX program types
static __forceinline__ __device__ void optixGetMicroTriangleVertexData(
    float3 data[3]);

/// Returns the barycentrics of the vertices of the currently intersected micro
/// triangle with respect to the base triangle.
///
/// Available in all OptiX program types
static __forceinline__ __device__ void optixGetMicroTriangleBarycentricsData(
    float2 data[3]);

/// Return the object space curve control vertex data of a linear curve in a
/// Geometry Acceleration Structure (GAS) at a given motion time.
///
/// To access vertex data, the GAS must be built using the flag
/// OPTIX_BUILD_FLAG_ALLOW_RANDOM_VERTEX_ACCESS.
///
/// data[i] = {x,y,z,w} with {x,y,z} the position and w the radius of control
/// vertex i.
///
/// If motion is disabled via OptixPipelineCompileOptions::usesMotionBlur, or
/// the GAS does not contain motion, the time parameter is ignored.
...
/// Return the object space curve control vertex data of a quadratic BSpline
/// curve in a Geometry Acceleration Structure (GAS) at a given motion time.
///
/// To access vertex data, the GAS must be built using the flag
/// OPTIX_BUILD_FLAG_ALLOW_RANDOM_VERTEX_ACCESS.
///
...
/// Return the object space curve control vertex data of a cubic BSpline curve
/// in a Geometry Acceleration Structure (GAS) at a given motion time.
///
/// To access vertex data, the GAS must be built using the flag
/// OPTIX_BUILD_FLAG_ALLOW_RANDOM_VERTEX_ACCESS.
///
...
/// Return the object space curve control vertex data of a CatmullRom spline
/// curve in a Geometry Acceleration Structure (GAS) at a given motion time.
///
/// To access vertex data, the GAS must be built using the flag
/// OPTIX_BUILD_FLAG_ALLOW_RANDOM_VERTEX_ACCESS.
...
/// Return the object space curve control vertex data of a cubic Bezier curve in
/// a Geometry Acceleration Structure (GAS) at a given motion time.
///
/// To access vertex data, the GAS must be built using the flag
/// OPTIX_BUILD_FLAG_ALLOW_RANDOM_VERTEX_ACCESS.
///
...
/// Return the object space curve control vertex data of a ribbon (flat
/// quadratic BSpline) in a Geometry Acceleration Structure (GAS) at a given
/// motion time.
///
/// To access vertex data, the GAS must be built using the flag
/// OPTIX_BUILD_FLAG_ALLOW_RANDOM_VERTEX_ACCESS.
...
/// Return the object space sphere data, center point and radius, in a Geometry
/// Acceleration Structure (GAS) at a given motion time.
///
/// To access sphere data, the GAS must be built using the flag
/// OPTIX_BUILD_FLAG_ALLOW_RANDOM_VERTEX_ACCESS.
///
/// data[0] = {x,y,z,w} with {x,y,z} the position of the sphere center and w the
/// radius.
///
/// If motion is disabled via OptixPipelineCompileOptions::usesMotionBlur, or
/// the GAS does not contain motion, the time parameter is ignored.
...
/// Returns the traversable handle for the Geometry Acceleration Structure (GAS)
/// containing the current hit.
///
...
/// Returns the world-to-object transformation matrix resulting from the current
/// active transformation list.
///
/// The cost of this function may be proportional to the size of the
/// transformation list.
///
/// Available in IS, AH, CH
...
/// Returns the object-to-world transformation matrix resulting from the current
/// active transformation list.
///
/// The cost of this function may be proportional to the size of the
/// transformation list.
...
/// Transforms the point using world-to-object transformation matrix resulting
/// from the current active transformation list.
///
/// The cost of this function may be proportional to the size of the
/// transformation list.
///
...
/// Transforms the vector using world-to-object transformation matrix resulting
/// from the current active transformation list.
///
/// The cost of this function may be proportional to the size of the
/// transformation list.
...
/// Transforms the normal using world-to-object transformation matrix resulting
/// from the current active transformation list.
///
/// The cost of this function may be proportional to the size of the
/// transformation list.
...
/// Transforms the point using object-to-world transformation matrix resulting
/// from the current active transformation list.
///
/// The cost of this function may be proportional to the size of the
/// transformation list.
...
/// Transforms the vector using object-to-world transformation matrix resulting
/// from the current active transformation list.
///
/// The cost of this function may be proportional to the size of the
/// transformation list.
...
/// Transforms the normal using object-to-world transformation matrix resulting
/// from the current active transformation list.
///
/// The cost of this function may be proportional to the size of the
/// transformation list.
...
/// Returns the number of transforms associated with the current outgoing hit
/// object's transform list.
///
...
/// Returns the traversable handle for a transform in the current transform
/// list.
///
/// Available in IS, AH, CH
static __forceinline__ __device__ OptixTraversableHandle
...
/// Available in RG, CH, MS, CC, DC
static __forceinline__ __device__ OptixTraversableHandle
optixHitObjectGetTransformListHandle(unsigned int index);
...
///
/// Returns 0 if the traversable is not of type
...
/// Returns a pointer to a OptixMatrixMotionTransform from its traversable
/// handle.
///
/// Returns 0 if the traversable is not of type
...
/// Returns a pointer to the geometry acceleration structure from its
/// traversable handle.
///
/// Returns 0 if the traversable is not a geometry acceleration structure.
///
/// Available in all OptiX program types
static __device__ __forceinline__ CUdeviceptr
optixGetGASPointerFromHandle(OptixTraversableHandle handle);
/// Reports an intersections (overload without attributes).
///
/// If optixGetRayTmin() <= hitT <= optixGetRayTmax(), the any hit program
/// associated with this intersection program (via the SBT entry) is called.
///
/// The AH program can do one of three things:
/// 1. call optixIgnoreIntersection - no hit is recorded,
/// optixReportIntersection returns false
/// 2. call optixTerminateRay       -    hit is recorded,
/// optixReportIntersection does not return, no further traversal occurs,
///                                                       and the associated
///                                                       closest hit program is
///                                                       called
/// 3. neither                      -    hit is recorded,
/// optixReportIntersection returns true
///
/// hitKind - Only the 7 least significant bits should be written [0..127].  Any
/// values above 127 are reserved for built in intersection.  The value can be
/// queried with optixGetHitKind() in AH and CH.
///
/// The attributes specified with a0..a7 are available in the AH and CH
/// programs.  Note that the attributes available in the CH program correspond
/// to the closest recorded intersection.  The number of attributes in registers
/// and memory can be configured in the pipeline.
///
/// \param[in] hitT
/// \param[in] hitKind
///
...
/// Returns the attribute at the given slot index. There are up to 8 attributes
/// available. The number of attributes is configured with
...
/// Return the attribute at the given slot index for the current outgoing hit
/// object. There are up to 8 attributes available. The number of attributes is
/// configured with OptixPipelineCompileOptions::numAttributeValues.
///
/// Results are undefined if the hit object is a miss.
///
/// Available in RG, CH, MS, CC, DC
...
/// Discards the hit, and returns control to the calling optixReportIntersection
/// or built-in intersection routine.
///
/// Available in AH
static __forceinline__ __device__ void optixIgnoreIntersection();

/// For a given OptixBuildInputTriangleArray the number of primitives is defined
/// as
///
/// "(OptixBuildInputTriangleArray::indexBuffer == 0) ?
/// OptixBuildInputTriangleArray::numVertices/3 :
///                                                     OptixBuildInputTriangleArray::numIndexTriplets;".
///
/// For a given OptixBuildInputCustomPrimitiveArray the number of primitives is
/// defined as numAabbs.
///
...
///
/// In CH this corresponds to the primitive index of the closest intersected
/// primitive.
///
/// Available in IS, AH, CH
...
/// Returns the Sbt GAS index of the primitive associated with the current
/// intersection.
///
/// In IS and AH this corresponds to the currently intersected primitive.
///
/// In CH this corresponds to the SBT GAS index of the closest intersected
/// primitive.
///
/// Available in IS, AH, CH
static __forceinline__ __device__ unsigned int optixGetSbtGASIndex();

/// Return the SBT GAS index of the closest intersected primitive associated
/// with the current outgoing hit object.
///
/// Results are undefined if the hit object is a miss.
///
/// See #optixGetSbtGASIndex() for details on the version for the incoming hit
/// object.
///
/// Available in RG, CH, MS, CC, DC
static __forceinline__ __device__ unsigned int optixHitObjectGetSbtGASIndex();

/// Returns the OptixInstance::instanceId of the instance within the top level
/// acceleration structure associated with the current intersection.
///
/// When building an acceleration structure using OptixBuildInputInstanceArray
/// each OptixInstance has a user supplied instanceId.  OptixInstance objects
/// reference another acceleration structure. During traversal the acceleration
/// structures are visited top down.  In the IS and AH programs the
/// OptixInstance::instanceId corresponding to the most recently visited
/// OptixInstance is returned when calling optixGetInstanceId().  In CH
/// optixGetInstanceId() returns the OptixInstance::instanceId when the hit was
/// recorded with optixReportIntersection.  In the case where there is no
/// OptixInstance visited, optixGetInstanceId returns 0
///
/// Available in IS, AH, CH
static __forceinline__ __device__ unsigned int optixGetInstanceId();

/// Returns the OptixInstance::instanceId of the instance within the top level
/// acceleration structure associated with the outgoing hit object.
///
/// Results are undefined if the hit object is a miss.
///
...
/// Returns the zero-based index of the instance within its instance
/// acceleration structure associated with the current intersection.
///
/// In the IS and AH programs the index corresponding to the most recently
/// visited OptixInstance is returned when calling optixGetInstanceIndex().  In
/// CH optixGetInstanceIndex() returns the index when the hit was recorded with
/// optixReportIntersection.  In the case where there is no OptixInstance
...
///
/// Use optixGetPrimitiveType() to interpret the hit kind.  For custom
/// intersections (primitive type OPTIX_PRIMITIVE_TYPE_CUSTOM), this is the
/// 7-bit hitKind passed to optixReportIntersection(). Hit kinds greater than
/// 127 are reserved for built-in primitives.
///
...
///
/// Available in all OptiX program types
static __forceinline__ __device__ OptixPrimitiveType
...
/// Function interpreting the hit kind associated with the current
...
///
/// When using OptixBuildInputTriangleArray objects, during intersection the
/// barycentric coordinates are stored into the first two attribute registers.
///
/// Available in AH, CH
static __forceinline__ __device__ float2 optixGetTriangleBarycentrics();

/// Returns the curve parameter associated with the current intersection when
/// using OptixBuildInputCurveArray objects.
///
/// Available in AH, CH
static __forceinline__ __device__ float optixGetCurveParameter();

/// Returns the ribbon parameters along directrix (length) and generator (width)
/// of the current intersection when using OptixBuildInputCurveArray objects
/// with curveType OPTIX_PRIMITIVE_TYPE_FLAT_QUADRATIC_BSPLINE.
///
/// Available in AH, CH
static __forceinline__ __device__ float2 optixGetRibbonParameters();

/// Available in any program, it returns the current launch index within the
/// launch dimensions specified by optixLaunch on the host.
///
...
/// Available in any program, it returns the dimensions of the current launch
/// specified by optixLaunch on the host.
///
/// Available in all OptiX program types
static __forceinline__ __device__ uint3 optixGetLaunchDimensions();

/// Returns the generic memory space pointer to the data region (past the
/// header) of the currently active SBT record corresponding to the current
/// program.
///
/// Note that optixGetSbtDataPointer is not available in OptiX-enabled
/// functions, because there is no SBT entry associated with the function.
///
/// Available in RG, IS, AH, CH, MS, EX, DC, CC
static __forceinline__ __device__ CUdeviceptr optixGetSbtDataPointer();

/// Device pointer address for the SBT associated with the hit or miss program
/// for the current outgoing hit object.
///
/// Returns 0 for nop hit objects.
///
/// Available in RG, CH, MS, CC, DC
...
/// Throws a user exception with the given exception code (overload without
/// exception details).
///
/// The exception code must be in the range from 0 to 2^30 - 1. Up to 8 optional
/// exception details can be passed. They can be queried in the EX program using
/// optixGetExceptionDetail_0() to
/// ..._8().
///
/// The exception details must not be used to encode pointers to the stack since
/// the current stack is not preserved in the EX program.
///
/// Not available in EX
///
...
/// Throws a user exception with the given exception code (overload with 1
/// exception detail).
///
...
/// Throws a user exception with the given exception code (overload with 2
/// exception details).
///
/// \see #optixThrowException(int)
///
...
/// Throws a user exception with the given exception code (overload with 3
/// exception details).
///
/// \see #optixThrowException(int)
///
...
/// Throws a user exception with the given exception code (overload with 4
/// exception details).
///
/// \see #optixThrowException(int)
///
...
/// Throws a user exception with the given exception code (overload with 5
/// exception details).
///
/// \see #optixThrowException(int)
///
...
/// Throws a user exception with the given exception code (overload with 6
/// exception details).
///
/// \see #optixThrowException(int)
///
...
/// Throws a user exception with the given exception code (overload with 7
/// exception details).
///
/// \see #optixThrowException(int)
///
...
/// Throws a user exception with the given exception code (overload with 8
/// exception details).
///
/// \see #optixThrowException(int)
///
...
///
/// The behavior is undefined if the exception is not a user exception, or the
/// used overload #optixThrowException() did not provide the queried exception
/// detail.
///
/// Available in EX
...
/// Returns a string that includes information about the source location that
...
/// OptixModuleCompileOptions::debugLevel may not be set to
...
///
/// Direct callable programs are allowed to call optixTrace, but any secondary
/// trace calls invoked from subsequently called CH, MS and callable programs
/// will result an an error.
///
/// Behavior is undefined if there is no direct callable program at the
/// specified SBT entry.
///
/// Behavior is undefined if the number of arguments that are being passed in
/// does not match the number of parameters expected by the program that is
/// called. In validation mode an exception will be generated.
///
/// \param[in] sbtIndex The offset of the SBT entry of the direct callable
/// program to call relative to OptixShaderBindingTable::callablesRecordBase.
/// \param[in] args The arguments to pass to the direct callable program.
///
/// Available in RG, IS, AH, CH, MS, DC, CC
template<typename ReturnT, typename... ArgTypes>
static __forceinline__ __device__ ReturnT
optixDirectCall(unsigned int sbtIndex, ArgTypes... args);

/// Creates a call to the continuation callable program at the specified SBT
/// entry.
///
/// This will call the program that was specified in the
...
///  OptixShaderBindingTable::callablesRecordStrideInBytes * sbtIndex ).
///
/// As opposed to direct callable programs, continuation callable programs are
/// allowed to make secondary optixTrace calls.
///
/// Behavior is undefined if there is no continuation callable program at the
/// specified SBT entry.
///
/// Behavior is undefined if the number of arguments that are being passed in
/// does not match the number of parameters expected by the program that is
/// called. In validation mode an exception will be generated.
///
/// \param[in] sbtIndex The offset of the SBT entry of the continuation callable
/// program to call relative to OptixShaderBindingTable::callablesRecordBase.
...
/// optixTexFootprint2D calculates the footprint of a corresponding 2D texture
/// fetch (non-mipmapped).
///
/// On Turing and subsequent architectures, a texture footprint instruction
/// allows user programs to determine the set of texels that would be accessed
/// by an equivalent filtered texture lookup.
///
/// \param[in] tex      CUDA texture object (cast to 64-bit integer)
/// \param[in] texInfo  Texture info packed into 32-bit integer, described
/// below.
/// \param[in] x        Texture coordinate
/// \param[in] y        Texture coordinate
/// \param[out] singleMipLevel  Result indicating whether the footprint spans
/// only a single miplevel.
///
/// The texture info argument is a packed 32-bit integer with the following
/// layout:
///
...
///
/// Returns a 16-byte structure (as a uint4) that stores the footprint of a
/// texture request at a particular "granularity", which has the following
/// layout:
///
...
///    };
///
/// The granularity indicates the size of texel groups that are represented by
/// an 8x8 bitmask. For example, a granularity of 12 indicates texel groups that
/// are 128x64 texels in size. In a footprint call, The returned granularity
/// will either be the actual granularity of the result, or 0 if the footprint
/// call was able to honor the requested granularity (the usual case).
///
/// level is the mip level of the returned footprint. Two footprint calls are
/// needed to get the complete footprint when a texture call spans multiple mip
/// levels.
///
/// mask is an 8x8 bitmask of texel groups that are covered, or partially
/// covered, by the footprint. tileX and tileY give the starting position of the
/// mask in 8x8 texel-group blocks.  For example, suppose a granularity of 12
/// (128x64 texels), and tileX=3 and tileY=4. In this case, bit 0 of the mask
/// (the low order bit) corresponds to texel group coordinates (3*8, 4*8), and
/// texel coordinates (3*8*128, 4*8*64), within the specified mip level.
///
/// If nonzero, dx and dy specify a "toroidal rotation" of the bitmask. Toroidal
/// rotation of a coordinate in the mask simply means that its value is reduced
/// by 8.  Continuing the example from above, if dx=0 and dy=0 the mask covers
/// texel groups (3*8, 4*8) to (3*8+7, 4*8+7) inclusive. If, on the other hand,
/// dx=2, the rightmost 2 columns in the mask have their x coordinates reduced
/// by 8, and similarly for dy.
///
...
/// optixTexFootprint2DLod calculates the footprint of a corresponding 2D
/// texture fetch (tex2DLod)
/// \param[in] tex      CUDA texture object (cast to 64-bit integer)
/// \param[in] texInfo  Texture info packed into 32-bit integer, described
/// below.
/// \param[in] x        Texture coordinate
...
/// \param[in] coarse   Requests footprint from coarse miplevel, when the
/// footprint spans two levels.
/// \param[out] singleMipLevel  Result indicating whether the footprint spans
/// only a single miplevel.
/// \see #optixTexFootprint2D(unsigned long long,unsigned
/// int,float,float,unsigned int*)
///
/// Available anywhere
...
/// optixTexFootprint2DGrad calculates the footprint of a corresponding 2D
/// texture fetch (tex2DGrad)
/// \param[in] tex      CUDA texture object (cast to 64-bit integer)
/// \param[in] texInfo  Texture info packed into 32-bit integer, described
/// below.
/// \param[in] x        Texture coordinate
/// \param[in] y        Texture coordinate
/// \param[in] dPdx_x   Derivative of x coordinte, which determines level of
/// detail.
/// \param[in] dPdx_y   Derivative of x coordinte, which determines level of
/// detail.
/// \param[in] dPdy_x   Derivative of y coordinte, which determines level of
/// detail.
/// \param[in] dPdy_y   Derivative of y coordinte, which determines level of
/// detail.
/// \param[in] coarse   Requests footprint from coarse miplevel, when the
/// footprint spans two levels.
/// \param[out] singleMipLevel  Result indicating whether the footprint spans
