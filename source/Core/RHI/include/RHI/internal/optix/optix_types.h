...
// This typedef should match the one in cuda.h in order to avoid compilation
// errors.
...
/// Alignment requirement for output and temporay buffers for acceleration
/// structures.
...
/// OptixOpacityMicromapArrayBuildInput::perMicromapDescBuffer.
...
/// Opacity micromaps encode the states of microtriangles in either 1 bit
/// (2-state) or 2 bits (4-state) using the following values.
...
/// Predefined index to indicate that a triangle in the BVH build doesn't have
/// an associated opacity micromap, and that it should revert to one of the four
/// possible states for the full triangle.
...
typedef enum OptixDeviceProperty {
    /// Maximum value for OptixPipelineLinkOptions::maxTraceDepth. sizeof(
    /// unsigned int )
    ...
    /// The maximum number of instances (over all build inputs) as input to a
    ...
    /// Returns a flag specifying capabilities of the optixReorder() device
    /// function.  See OptixDevicePropertyShaderExecutionReorderingFlags for
    /// documentation on the values that can be returned. sizeof( unsigned int )
...
/// STAT').
/// \param[in] message    Null terminated log message (without newline at the
/// end).
/// \param[in] cbdata     Callback data that was provided with the callback
/// pointer.
///
...
typedef struct OptixDeviceContextOptions {
    ...
    /// Pointer stored and passed to logCallbackFunction when a message is
    /// generated
...
typedef enum OptixDevicePropertyShaderExecutionReorderingFlags {
    /// optixReorder() acts as a no-op, and no thread reordering is performed.
    /// Note that it is still legal to call this device function; no errors will
    /// be generated.
...
typedef enum OptixIndicesFormat {
    /// No indices, this format must only be used in combination with triangle
...
typedef enum OptixVertexFormat {
    ...
    OPTIX_VERTEX_FORMAT_FLOAT3 =
        0x2121,  ///< Vertices are represented by three floats
    OPTIX_VERTEX_FORMAT_FLOAT2 =
        0x2122,  ///< Vertices are represented by two floats
    OPTIX_VERTEX_FORMAT_HALF3 =
        0x2123,  ///< Vertices are represented by three halfs
    OPTIX_VERTEX_FORMAT_HALF2 =
        0x2124,  ///< Vertices are represented by two halfs
...
typedef enum OptixTransformFormat {
    OPTIX_TRANSFORM_FORMAT_NONE =
        0,  ///< no transform, default for zero initialization
    OPTIX_TRANSFORM_FORMAT_MATRIX_FLOAT12 =
        0x21E1,  ///< 3x4 row major affine matrix
...
typedef enum OptixOpacityMicromapArrayIndexingMode {
    ...
    /// opacity micromap array is used. triangle[i] will use
    /// opacityMicromapArray[i].
    OPTIX_OPACITY_MICROMAP_ARRAY_INDEXING_MODE_LINEAR = 1,
    /// OptixBuildInputOpacityMicromap::indexBuffer provides a per triangle
    /// array of predefined indices and/or indices into
    /// OptixBuildInputOpacityMicromap::opacityMicromapArray. See
    /// OptixBuildInputOpacityMicromap::indexBuffer for more details.
...
/// Specifies how many opacity micromaps of a specific type are referenced by
/// triangles when building the AS. Note that while this is similar to
/// OptixOpacityMicromapHistogramEntry, the usage count specifies how many
/// opacity micromaps of a specific type are referenced by triangles in the AS.
typedef struct OptixOpacityMicromapUsageCount {
    /// Number of opacity micromaps with this format and subdivision level
    /// referenced by triangles in the corresponding triangle build input at AS
    /// build time.
    unsigned int count;
...
typedef struct OptixBuildInputOpacityMicromap {
    ...
    /// Device pointer to a opacity micromap array used by this build input
    /// array. This buffer is required when
    /// #OptixBuildInputOpacityMicromap::indexingMode is
    /// OPTIX_OPACITY_MICROMAP_ARRAY_INDEXING_MODE_LINEAR or
    /// OPTIX_OPACITY_MICROMAP_ARRAY_INDEXING_MODE_INDEXED. Must be zero if
    /// #OptixBuildInputOpacityMicromap::indexingMode is
    /// OPTIX_OPACITY_MICROMAP_ARRAY_INDEXING_MODE_NONE.
    CUdeviceptr opacityMicromapArray;

    /// int16 or int32 buffer specifying which opacity micromap index to use for
    /// each triangle. Instead of an actual index, one of the predefined indices
    /// OPTIX_OPACITY_MICROMAP_PREDEFINED_INDEX_(FULLY_TRANSPARENT |
    /// FULLY_OPAQUE | FULLY_UNKNOWN_TRANSPARENT | FULLY_UNKNOWN_OPAQUE) can be
    /// used to indicate that there is no opacity micromap for this particular
    /// triangle but the triangle is in a uniform state and the selected
    /// behavior is applied to the entire triangle. This buffer is required when
    /// #OptixBuildInputOpacityMicromap::indexingMode is
    /// OPTIX_OPACITY_MICROMAP_ARRAY_INDEXING_MODE_INDEXED. Must be zero if
    /// #OptixBuildInputOpacityMicromap::indexingMode is
    /// OPTIX_OPACITY_MICROMAP_ARRAY_INDEXING_MODE_LINEAR or
    ...
    /// Must be non-zero when #OptixBuildInputOpacityMicromap::indexingMode is
    /// OPTIX_OPACITY_MICROMAP_ARRAY_INDEXING_MODE_INDEXED.
    unsigned int indexSizeInBytes;

    /// Opacity micromap index buffer stride. If set to zero, indices are
    /// assumed to be tightly packed and stride is inferred from
    /// #OptixBuildInputOpacityMicromap::indexSizeInBytes.
    ...
    /// List of number of usages of opacity micromaps of format and subdivision
    /// combinations. Counts with equal format and subdivision combination
    /// (duplicates) are added together.
    const OptixOpacityMicromapUsageCount* micromapUsageCounts;
} OptixBuildInputOpacityMicromap;

typedef struct OptixRelocateInputOpacityMicromap {
    /// Device pointer to a relocated opacity micromap array used by the source
    /// build input array. May be zero when no micromaps where used in the
    /// source accel, or the referenced opacity micromaps don't require
    /// relocation (for example relocation of a GAS on the source device).
    CUdeviceptr opacityMicromapArray;
...
typedef enum OptixDisplacementMicromapTriangleFlags {
    ...
    /// The triangle edge v0..v1 is decimated: after subdivision the number of
    /// micro triangles on that edge is halved such that a neighboring triangle
    /// can have a lower subdivision level without introducing cracks.
...
/// Specifies how many displacement micromaps of a specific type are input to
/// the displacement micromap array build. Note that while this is similar to
/// OptixDisplacementMicromapUsageCount, the histogram entry specifies how many
/// displacement micromaps of a specific type are combined into a displacement
/// micromap array.
typedef struct OptixDisplacementMicromapHistogramEntry {
    /// Number of displacement micromaps with the format and subdivision level
    /// that are input to the displacement micromap array build.
    unsigned int count;
    /// Number of micro-triangles is 4^level. Valid levels are [0, 5]
    unsigned int subdivisionLevel;
...
typedef struct OptixDisplacementMicromapArrayBuildInput {
    ...
    /// 128 byte aligned pointer for displacement values input data (the
    /// displacement blocks).
    CUdeviceptr displacementValuesBuffer;
    /// Descriptors for interpreting displacement values input data, one
    /// OptixDisplacementMicromapDesc entry required per displacement micromap.
    /// This device pointer must be a multiple of
    /// OPTIX_DISPLACEMENT_MICROMAP_DESC_BUFFER_BYTE_ALIGNMENT.
    CUdeviceptr perDisplacementMicromapDescBuffer;
    /// Stride between OptixDisplacementMicromapDesc in
    /// perDisplacementMicromapDescBuffer If set to zero, the displacement
    /// micromap descriptors are assumed to be tightly packed and the stride is
    /// assumed to be sizeof( OptixDisplacementMicromapDesc ). This stride must
    /// be a multiple of OPTIX_DISPLACEMENT_MICROMAP_DESC_BUFFER_BYTE_ALIGNMENT.
...
/// Specifies how many displacement micromaps of a specific type are referenced
/// by triangles when building the AS. Note that while this is similar to
/// OptixDisplacementMicromapHistogramEntry, the usage count specifies how many
/// displacement micromaps of a specific type are referenced by triangles in the
/// AS.
typedef struct OptixDisplacementMicromapUsageCount {
    /// Number of displacement micromaps with this format and subdivision level
    /// referenced by triangles in the corresponding triangle build input at AS
    /// build time.
    unsigned int count;
    /// Number of micro-triangles is 4^level. Valid levels are [0, 5]
...
/// indexing mode of triangles to displacement micromaps in an array, used in
...
typedef enum OptixDisplacementMicromapArrayIndexingMode {
    ...
    /// displacement micromap array is used. triangle[i] will use
    /// displacementMicromapArray[i].
    ...
    /// provides a per triangle array of indices into
    /// OptixBuildInputDisplacementMicromap::displacementMicromapArray. See
    /// OptixBuildInputDisplacementMicromap::displacementMicromapIndexBuffer for
    /// more details.
...
typedef struct OptixBuildInputDisplacementMicromap {
    ...
    /// Address to a displacement micromap array used by this build input array.
    /// Set to NULL to disable DMs for this input.
    CUdeviceptr displacementMicromapArray;
    /// int16 or int32 buffer specifying which displacement micromap index to
    /// use for each triangle. Only valid if displacementMicromapArray != NULL.
    CUdeviceptr displacementMicromapIndexBuffer;
    /// Per triangle-vertex displacement directions.
    CUdeviceptr vertexDirectionsBuffer;
    /// Optional per-vertex bias (offset) along displacement direction and
    /// displacement direction scale.
    CUdeviceptr vertexBiasAndScaleBuffer;
    /// Optional per-triangle flags, uint8_t per triangle, possible values
    /// defined in enum OptixDisplacementMicromapTriangleFlags
    CUdeviceptr triangleFlagsBuffer;

    /// Constant offset to displacement micromap indices as specified by the
    /// displacement micromap index buffer
    unsigned int displacementMicromapIndexOffset;
    /// Displacement micromap index buffer stride. If set to zero, indices are
    /// assumed to be tightly packed and stride is inferred from
    /// #OptixBuildInputDisplacementMicromap::displacementMicromapIndexSizeInBytes.
    ...
    /// List of number of usages of displacement micromaps of format and
    /// subdivision combinations. Counts with equal format and subdivision
    /// combination (duplicates) are added together.
...
typedef struct OptixBuildInputTriangleArray {
    /// Points to host array of device pointers, one per motion step. Host array
    ...
    /// alignment must match the natural alignment of the type as specified in
    /// the vertexFormat, i.e., for OPTIX_VERTEX_FORMAT_FLOATX 4-byte, for all
    /// others a 2-byte alignment. However, an 16-byte stride (and buffer
    /// alignment) is recommended for vertices of format
    /// OPTIX_VERTEX_FORMAT_FLOAT3 for GAS build performance.
    const CUdeviceptr* vertexBuffers;

    /// Number of vertices in each of buffer in
    ...
    /// Stride between vertices. If set to zero, vertices are assumed to be
    /// tightly packed and stride is inferred from vertexFormat.
    unsigned int vertexStrideInBytes;

    /// Optional pointer to array of 16 or 32-bit int triplets, one triplet per
    ...
    /// Size of array in OptixBuildInputTriangleArray::indexBuffer. For build,
    ...
    /// Stride between triplets of indices. If set to zero, indices are assumed
    /// to be tightly packed and stride is inferred from indexFormat.
    unsigned int indexStrideInBytes;
    ...
    /// Device pointer to per-primitive local sbt index offset buffer. May be
    /// NULL. Every entry must be in range [0,numSbtRecords-1]. Size needs to be
    /// the number of primitives.
    CUdeviceptr sbtIndexOffsetBuffer;

    /// Size of type of the sbt index offset. Needs to be 0, 1, 2 or 4 (8, 16 or
    /// 32 bit).
    unsigned int sbtIndexOffsetSizeInBytes;

    /// Stride between the index offsets. If set to zero, the offsets are
...
typedef struct OptixRelocateInputTriangleArray {
    /// Number of sbt records available to the sbt index offset override.
    /// Must match #OptixBuildInputTriangleArray::numSbtRecords of the source
    /// build input.
    unsigned int numSbtRecords;
...
typedef enum OptixCurveEndcapFlags {
    /// Default end caps. Round end caps for linear, no end caps for
    /// quadratic/cubic.
    OPTIX_CURVE_ENDCAP_DEFAULT = 0,
    /// Flat end caps at both ends of quadratic/cubic curve segments. Not valid
    /// for linear.
...
typedef struct OptixBuildInputCurveArray {
    ...
    /// Pointer to host array of device pointers, one per motion step. Host
    /// array size must match number of motion keys as set in
    /// #OptixMotionOptions (or an array of size 1 if
    /// OptixMotionOptions::numKeys is set to 1). Each per-motion-key device
    /// pointer must point to an array of floats (the vertices of the curves).
    const CUdeviceptr* vertexBuffers;
    ...
    /// Parallel to vertexBuffers: a device pointer per motion step, each with
    ...
    /// Each index is the start of degree+1 consecutive vertices in
    /// vertexBuffers, and corresponding widths in widthBuffers and normals in
    /// normalBuffers. These define a single segment. Size of array is
    /// numPrimitives.
    CUdeviceptr indexBuffer;
    /// Stride between indices. If set to zero, indices are assumed to be
    /// tightly packed and stride is sizeof( unsigned int ).
    unsigned int indexStrideInBytes;
...
typedef struct OptixBuildInputSphereArray {
    ...
    /// #OptixMotionOptions (or an array of size 1 if
    /// OptixMotionOptions::numKeys is set to 1). Each per-motion-key device
    /// pointer must point to an array of floats (the center points of the
    /// spheres).
    const CUdeviceptr* vertexBuffers;
    ...
    /// Parallel to vertexBuffers: a device pointer per motion step, each with
    /// numRadii float values, specifying the sphere radius corresponding to
    /// each vertex.
    ...
    /// Boolean value indicating whether a single radius per radius buffer is
    ...
    /// 32 bit).
    unsigned int sbtIndexOffsetSizeInBytes;
    /// Stride between the sbt index offsets. If set to zero, the offsets are
    /// assumed to be tightly packed and the stride matches the size of the type
    /// (sbtIndexOffsetSizeInBytes).
    unsigned int sbtIndexOffsetStrideInBytes;
...
typedef struct OptixBuildInputCustomPrimitiveArray {
    /// Points to host array of device pointers to AABBs (type OptixAabb), one
    /// per motion step. Host array size must match number of motion keys as set
    /// in OptixMotionOptions (or an array of size 1 if
    /// OptixMotionOptions::numKeys is set to 1). Each device pointer must be a
    /// multiple of OPTIX_AABB_BUFFER_BYTE_ALIGNMENT.
    ...
    /// Stride between AABBs (per motion key). If set to zero, the aabbs are
...
typedef struct OptixBuildInputInstanceArray {
    ...
    /// instances and aabbs should be interpreted as arrays of pointers instead
    /// of arrays of structs.
    ///
    ...
    /// Defines the stride between instances. A stride of 0 indicates a tight
    /// packing, i.e., stride = sizeof( OptixInstance )
...
typedef struct OptixRelocateInputInstanceArray {
    ...
    /// #OptixRelocateInputInstanceArray::traversableHandles. Must match
    /// #OptixBuildInputInstanceArray::numInstances of the source build input.
    unsigned int numInstances;

    /// These are the traversable handles of the instances (See
    /// OptixInstance::traversableHandle) These can be used when also relocating
    /// the instances.  No updates to the bounds are performed.  Use
    /// optixAccelBuild to update the bounds. 'traversableHandles' may be zero
    /// when the traversables are not relocated (i.e. relocation of an IAS on
    /// the source device).
...
static_assert(
    sizeof(OptixBuildInput) == 8 + 1024,
    "OptixBuildInput has wrong size");
#endif
...
typedef enum OptixInstanceFlags {
    ...
    /// Force 4-state opacity micromaps to behave as 2-state opacity micromaps
    /// during traversal.
    OPTIX_INSTANCE_FLAG_FORCE_OPACITY_MICROMAP_2_STATE = 1u << 4,
    /// Don't perform opacity micromap query for this instance. GAS must be
    /// built with ALLOW_DISABLE_OPACITY_MICROMAPS for this to be valid. This
    /// flag overrides FORCE_OPACTIY_MIXROMAP_2_STATE instance and ray flags.
...
typedef struct OptixInstance {
    ...
    /// Application supplied ID. The maximal ID can be queried using
    /// OPTIX_DEVICE_PROPERTY_LIMIT_MAX_INSTANCE_ID.
    unsigned int instanceId;

    /// SBT record offset.
    /// In a traversable graph with multiple levels of instance acceleration
    /// structure (IAS) objects, offsets are summed together. The maximal SBT
    /// offset can be queried using OPTIX_DEVICE_PROPERTY_LIMIT_MAX_SBT_OFFSET.
    unsigned int sbtOffset;

    /// Visibility mask. If rayMask & instanceMask == 0 the instance is culled.
...
typedef enum OptixBuildFlags {
    ...
    /// Allow updating the build with new vertex positions with subsequent calls
    /// to optixAccelBuild.
    ...
    /// Support updating the opacity micromap array and opacity micromap indices
    /// on refits. May increase AS size and may have a small negative impact on
    /// traversal performance. If this flag is absent, all opacity micromap
    /// inputs must remain unchanged between the initial AS builds and their
    /// subsequent refits.
    OPTIX_BUILD_FLAG_ALLOW_OPACITY_MICROMAP_UPDATE = 1u << 6,

    /// If enabled, any instances referencing this GAS are allowed to disable
    /// the opacity micromap test through the DISABLE_OPACITY_MICROMAPS flag
    /// instance flag. Note that the GAS will not be optimized for the attached
    /// opacity micromap Arrays if this flag is set, which may result in reduced
...
typedef struct OptixOpacityMicromapDesc {
    /// Byte offset to opacity micromap in data input buffer of opacity micromap
    /// array build
    unsigned int byteOffset;
    /// Number of micro-triangles is 4^level. Valid levels are [0, 12]
    unsigned short subdivisionLevel;
...
/// Specifies how many opacity micromaps of a specific type are input to the
/// opacity micromap array build. Note that while this is similar to
/// OptixOpacityMicromapUsageCount, the histogram entry specifies how many
/// opacity micromaps of a specific type are combined into a opacity micromap
/// array.
typedef struct OptixOpacityMicromapHistogramEntry {
    /// Number of opacity micromaps with the format and subdivision level that
    /// are input to the opacity micromap array build.
    unsigned int count;
    /// Number of micro-triangles is 4^level. Valid levels are [0, 12].
    unsigned int subdivisionLevel;
    /// Opacity micromap format.
...
typedef struct OptixOpacityMicromapArrayBuildInput {
    ...
    /// If set to zero, the opacity micromap descriptors are assumed to be
    /// tightly packed and the stride is assumed to be sizeof(
    /// OptixOpacityMicromapDesc ). This stride must be a multiple of
    /// OPTIX_OPACITY_MICROMAP_DESC_BYTE_ALIGNMENT.
    ...
    /// Histogram over opacity micromaps of input format and subdivision
    /// combinations. Counts of entries with equal format and subdivision
    /// combination (duplicates) are added together.
    const OptixOpacityMicromapHistogramEntry* micromapHistogramEntries;
} OptixOpacityMicromapArrayBuildInput;

/// Conservative memory requirements for building a opacity/displacement
/// micromap array
...
typedef struct OptixAccelBuildOptions {
    ...
    /// If OPTIX_BUILD_OPERATION_UPDATE the output buffer is assumed to contain
    ...
    /// reflect the current position of the primitives. If a BLAS has been built
    /// with OPTIX_BUILD_FLAG_ALLOW_OPACITY_MICROMAP_UPDATE, new opacity
    /// micromap arrays and opacity micromap indices may be provided to the
    /// refit.
...
typedef struct OptixAccelBufferSizes {
    /// The size in bytes required for the outputBuffer parameter to
    /// optixAccelBuild when doing a build (OPTIX_BUILD_OPERATION_BUILD).
    size_t outputSizeInBytes;

    /// The size in bytes required for the tempBuffer paramter to
    /// optixAccelBuild when doing a build (OPTIX_BUILD_OPERATION_BUILD).
    size_t tempSizeInBytes;

    /// The size in bytes required for the tempBuffer parameter to
    /// optixAccelBuild when doing an update (OPTIX_BUILD_OPERATION_UPDATE).
    /// This value can be different than tempSizeInBytes used for a full build.
    /// Only non-zero if OPTIX_BUILD_FLAG_ALLOW_UPDATE flag is set in
    /// OptixAccelBuildOptions.
...
typedef enum OptixAccelPropertyType {
    /// Size of a compacted acceleration structure. The device pointer points to
    /// a uint64.
...
/// #optixOpacityMicromapArrayRelocate(), #optixAccelGetRelocationInfo(),
/// #optixAccelRelocate(), #optixCheckRelocationCompatibility()
...
///
/// The scaling matrix
/// \f$S = \begin{bmatrix} sx & a & b & pvx \\ 0 & sy & c & pvy \\ 0 & 0  & sz &
/// pvz \end{bmatrix}\f$
//      [ sx   a   b  pvx ]
//  S = [  0  sy   c  pvy ]
//      [  0   0  sz  pvz ]
/// defines an affine transformation that can include scale, shear, and a
/// translation. The translation allows to define the pivot point for the
/// subsequent rotation.
///
/// The quaternion R = [ qx, qy, qz, qw ] describes a rotation  with angular
/// component qw = cos(theta/2) and other components [ qx, qy, qz ] =
/// sin(theta/2) * [ ax, ay, az ] where the axis [ ax, ay, az ] is normalized.
///
/// The translation matrix
/// \f$T = \begin{bmatrix} 1 & 0 & 0 & tx \\ 0 & 1 & 0 & ty \\ 0 & 0 & 1 & tz
/// \end{bmatrix}\f$
//      [  1  0  0 tx ]
//  T = [  0  1  0 ty ]
//      [  0  0  1 tz ]
/// defines another translation that is applied after the rotation. Typically,
...
#if defined(__cplusplus) && __cplusplus >= 201103L
static_assert(
    sizeof(OptixSRTMotionTransform) == 8 + 12 + 12 + 2 * 16 * 4,
    "OptixSRTMotionTransform has wrong size");
#endif
...
typedef struct OptixImage2D {
    ...
    /// For pixel format OPTIX_PIXEL_FORMAT_INTERNAL_GUIDE_LAYER it must be set
    /// to OptixDenoiserSizes::internalGuideLayerPixelSizeInBytes.
...
typedef enum OptixDenoiserModelKind {
    ...
    /// Use the built-in model appropriate for high dynamic range input and
    /// support for AOVs
...
typedef struct OptixDenoiserGuideLayer {
    ...
    // (X, Y) camera space for OPTIX_DENOISER_MODEL_KIND_LDR,
    // OPTIX_DENOISER_MODEL_KIND_HDR models. (X, Y, Z) world space, all other
    // models.
    OptixImage2D normal;

    // image with two components: X, Y.
    // pixel movement from previous to current frame for each pixel in screen
    // space.
    OptixImage2D flow;
    ...
    // image with a single component value that specifies how trustworthy the
    // flow vector at x,y position in OptixDenoiserGuideLayer::flow is. Range
    // 0..1 (low->high trustworthiness). Ignored if data pointer in the image is
    // zero.
...
typedef struct OptixDenoiserLayer {
    ...
    // Type of AOV, used in temporal AOV modes as a hint to improve image
    // quality.
...
typedef struct OptixDenoiserParams {
    /// average log intensity of input image (default null pointer). points to a
    /// single float. if set to null, autoexposure will be calculated
    /// automatically for the input image. Should be set to average log
    /// intensity of the entire image at least if tiling is used to get
    /// consistent autoexposure for all tiles.
    ...
    /// this parameter is used when the OPTIX_DENOISER_MODEL_KIND_AOV model kind
    /// is set. average log color of input image, separate for RGB channels
    /// (default null pointer). points to three floats. if set to null, average
    /// log color will be calculated automatically. See hdrIntensity for tiling,
    /// this also applies here.
    CUdeviceptr hdrAverageColor;

    /// In temporal modes this parameter must be set to 1 if previous layers
    /// (e.g. previousOutputInternalGuideLayer) contain valid data. This is the
    /// case in the second and subsequent frames of a sequence (for example
    /// after a change of camera angle). In the first frame of such a sequence
...
typedef struct OptixDenoiserSizes {
    /// Size of state memory passed to #optixDenoiserSetup,
    /// #optixDenoiserInvoke.
    size_t stateSizeInBytes;

    /// Size of scratch memory passed to #optixDenoiserSetup,
...
/// Ray flags passed to the device function #optixTrace().  These affect the
...
typedef enum OptixRayFlags {
    ...
    /// Disables closesthit programs for the ray, but still executes miss
    ...
    /// Force 4-state opacity micromaps to behave as 2-state opactiy micromaps
    /// during traversal.
...
typedef enum OptixTransformType {
    ...
    OPTIX_TRANSFORM_TYPE_MATRIX_MOTION_TRANSFORM =
        2,  ///< \see #OptixMatrixMotionTransform
    OPTIX_TRANSFORM_TYPE_SRT_MOTION_TRANSFORM =
        3,                              ///< \see #OptixSRTMotionTransform
...
typedef enum OptixTraversableGraphFlags {
    ...
    ///  Used to signal that a traversable graph of a single Geometry
    ///  Acceleration Structure (GAS) without any transforms is valid. This flag
    ///  may be combined with other flags except for
    ///  OPTIX_TRAVERSABLE_GRAPH_FLAG_ALLOW_ANY.
    OPTIX_TRAVERSABLE_GRAPH_FLAG_ALLOW_SINGLE_GAS = 1u << 0,

    ///  Used to signal that a traversable graph of a single Instance
    ///  Acceleration Structure (IAS) directly connected to Geometry
    ///  Acceleration Structure (GAS) traversables without transform
    ///  traversables in between is valid.  This flag may be combined with other
    ///  flags except for OPTIX_TRAVERSABLE_GRAPH_FLAG_ALLOW_ANY.
...
typedef enum OptixModuleCompileState {
    ...
    /// All OptixTask objects have completed. The OptixModule is ready to be
    /// used.
...
/// to 0, an OPTIX_ERROR_INVALID_VALUE will be returned from optixModuleCreate.
///
...
typedef struct OptixModuleCompileBoundValueEntry {
    ...
    const char* annotation;  // optional string to display, set to 0 if unused.
                             // If unused, OptiX will report the annotation as
                             // "No annotation"
...
typedef struct OptixPayloadType {
    ...
    /// Points to host array of payload word semantics, size must match
    /// numPayloadValues
...
typedef struct OptixModuleCompileOptions {
    ...
    /// Must be zero if OptixPipelineCompileOptions::numPayloadValues is not
    /// zero.
...
typedef enum OptixProgramGroupKind {
    ...
    /// Program group containing an intersection (IS), any hit (AH), and/or
    /// closest hit (CH) program
    /// \see #OptixProgramGroupSingleModule, #OptixProgramGroupDesc::hitgroup
    OPTIX_PROGRAM_GROUP_KIND_HITGROUP = 0x2424,

    /// Program group containing a direct (DC) or continuation (CC) callable
    /// program
...
typedef enum OptixExceptionFlags {
    ...
    /// This flag should be used when the application handles trace depth
    /// overflows in a user exception program as part of the normal flow of
    /// execution. For catching overflows during debugging and development, the
    /// device context validation mode should be used instead.
    /// \see OptixDeviceContextValidationMode
    OPTIX_EXCEPTION_FLAG_TRACE_DEPTH = 1u << 1,

    /// Enables user exceptions via optixThrowException(). This flag must be
...
typedef struct OptixPipelineCompileOptions {
    ...
    /// How much storage, in 32b words, to make available for the payload,
    /// [0..32] Must be zero if numPayloadTypes is not zero.
    int numPayloadValues;

    /// How much storage, in 32b words, to make available for the attributes.
    /// The minimum number is 2. Values below that will automatically be changed
    /// to 2. [2..8]
    int numAttributeValues;

    /// A bitmask of OptixExceptionFlags indicating which exceptions are
    /// enabled.
    unsigned int exceptionFlags;

    /// The name of the pipeline parameter variable.  If 0, no pipeline
    /// parameter will be available. This will be ignored if the launch param
    /// variable was optimized out or was not found in the modules linked to the
    /// pipeline.
    const char* pipelineLaunchParamsVariableName;

    /// Bit field enabling primitive types. See OptixPrimitiveTypeFlags.
    /// Setting to zero corresponds to enabling
    /// OPTIX_PRIMITIVE_TYPE_FLAGS_CUSTOM and
...
typedef struct OptixShaderBindingTable {
    /// Device address of the SBT record of the ray gen program to start launch
    /// at. The address must be a multiple of OPTIX_SBT_RECORD_ALIGNMENT.
    CUdeviceptr raygenRecord;

    /// Device address of the SBT record of the exception program. The address
    /// must be a multiple of OPTIX_SBT_RECORD_ALIGNMENT.
    CUdeviceptr exceptionRecord;

    /// Arrays of SBT records for miss programs. The base address and the stride
    /// must be a multiple of OPTIX_SBT_RECORD_ALIGNMENT.
    /// @{
    ...
    /// Arrays of SBT records for hit groups. The base address and the stride
    /// must be a multiple of OPTIX_SBT_RECORD_ALIGNMENT.
    /// @{
    ...
    /// Arrays of SBT records for callable programs. If the base address is not
    /// null, the stride and count must not be zero. If the base address is
    /// null, then the count needs to zero. The base address and the stride must
...
/// Specifies the options for retrieving an intersection program for a built-in
...
typedef struct OptixBuiltinISOptions {
    ...
    /// Boolean value indicating whether vertex motion blur is used (but not
    /// motion transform blur).
    int usesMotionBlur;
    /// Build flags, see OptixBuildFlags.
    unsigned int buildFlags;
    /// End cap properties of curves, see OptixCurveEndcapFlags, 0 for non-curve
    /// types.
