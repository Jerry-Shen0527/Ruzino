...
// mac os 32-bit still needs this
#if (                                                                     \
    (defined(macintosh) && defined(__LP64__) && (__NVAPI_RESERVED0__)) || \
...
NV_DECLARE_HANDLE(NvDisplayHandle);  //!< Display Device driven by NVIDIA GPU(s)
                                     //!< (an attached display)
...
typedef void* StereoHandle;  //!< A stereo handle, that corresponds to the
                             //!< device interface
...
NV_DECLARE_HANDLE(
    NvPresentBarrierClientHandle);  //!< PresentBarrier client object
//! @}
...
#define NV_MAX_VID_STREAMS_EX \
    20  //!< Increasing MAX no. of input video streams, each with a
        //!< #NVAPI_VIDEO_SRC_INFO
...
typedef enum _NvAPI_Status {
    ...
    NVAPI_NVIDIA_DEVICE_NOT_FOUND =
        -6,  //!< No NVIDIA display driver, or NVIDIA GPU driving a display, was
             //!< found.
    ...
    NVAPI_HANDLE_INVALIDATED =
        -10,  //!< The handle is no longer valid (likely due to GPU or display
              //!< re-configuration)
    ...
    NVAPI_SYSWOW64_NOT_SUPPORTED = -127,  //!< The function is not supported
                                          //!< from 32-bit on a 64-bit system.
    ...
    NVAPI_WAS_STILL_DRAWING =
        -131,  //!< The previous operation that is transferring information to
    ...
    NVAPI_TOO_MANY_UNIQUE_STATE_OBJECTS =
        -133,  //!< There are too many unique instances of a particular type of
               //!< state object.
    NVAPI_INVALID_CALL =
        -134,  //!< The method call is invalid. For example, a method's
    ...
    NVAPI_INVALID_USER_PRIVILEGE =
        -137,  //!< The application will require Administrator privileges to
               //!< access this API. The application can be elevated to a higher
    ...
    NVAPI_STEREO_NOT_ENABLED =
        -144,  //!< Stereo is not enabled and the function needed it to execute
               //!< completely.
    NVAPI_STEREO_NOT_TURNED_ON =
        -145,  //!< Stereo is not turned on and the function needed it to
               //!< execute completely.
    NVAPI_STEREO_INVALID_DEVICE_INTERFACE =
        -146,  //!< Invalid device interface.
    NVAPI_STEREO_PARAMETER_OUT_OF_RANGE =
        -147,  //!< Separation percentage or JPEG image capture quality is out
               //!< of [0-100] range.
    ...
    NVAPI_TOPO_NOT_POSSIBLE =
        -149,  //!< The mosaic topology is not possible given the current state
               //!< of the hardware.
    NVAPI_MODE_CHANGE_FAILED = -150,  //!< An attempt to do a display resolution
                                      //!< mode change has failed.
    ...
    NVAPI_MOSAIC_NOT_ACTIVE =
        -176,  //!< The requested action cannot be performed without Mosaic
               //!< being enabled.
    ...
    NVAPI_ECID_SIGN_ALGO_UNSUPPORTED =
        -197,  //!< The specified signing algorithm is not supported. Either an
               //!< incorrect value was entered or the current installed
    ...
    NVAPI_D3D_DEVICE_NOT_REGISTERED =
        -202,  //!< The user tried to use a deferred context without registering
               //!< the device first
    NVAPI_RESOURCE_NOT_ACQUIRED =
        -203,  //!< Head or SourceId was not reserved for the VR Display before
               //!< doing the Modeset or the dedicated display.
    NVAPI_TIMING_NOT_SUPPORTED = -204,  //!< Provided timing is not supported.
    NVAPI_HDCP_ENCRYPTION_FAILED =
        -205,  //!< HDCP Encryption Failed for the device. Would be applicable
               //!< when the device is HDCP Capable.
    NVAPI_PCLK_LIMITATION_FAILED =
        -206,  //!< Provided mode is over sink device pclk limitation.
    NVAPI_NO_CONNECTOR_FOUND = -207,  //!< No connector on GPU found.
    NVAPI_HDCP_DISABLED = -208,  //!< When a non-HDCP capable HMD is connected,
                                 //!< we would inform user by this code.
    NVAPI_API_IN_USE = -209,     //!< Atleast an API is still being called
    NVAPI_NVIDIA_DISPLAY_NOT_FOUND =
        -210,                         //!< No display found on Nvidia GPU(s).
    NVAPI_PRIV_SEC_VIOLATION = -211,  //!< Priv security violation, improper
                                      //!< access to a secured register.
    ...
    NVAPI_UNSUPPORTED_CONFIG_NON_HDCP_HMD =
        -214,  //!< The Config is having Non-NVidia GPU with Non-HDCP HMD
               //!< connected
    NVAPI_MAX_DISPLAY_LIMIT_REACHED =
        -215,  //!< GPU's Max Display Limit has Reached
    NVAPI_INVALID_DIRECT_MODE_DISPLAY =
        -216,  //!< DirectMode not Enabled on the Display
    NVAPI_GPU_IN_DEBUG_MODE =
        -217,  //!< GPU is in debug mode, OC is NOT allowed.
    NVAPI_D3D_CONTEXT_NOT_FOUND =
        -218,  //!< No NvAPI context was found for this D3D object
    NVAPI_STEREO_VERSION_MISMATCH =
        -219,  //!< there is version mismatch between stereo driver and dx
               //!< driver
    NVAPI_GPU_NOT_POWERED =
        -220,  //!< GPU is not powered and so the request cannot be completed.
    NVAPI_ERROR_DRIVER_RELOAD_IN_PROGRESS =
        -221,  //!< The display driver update in progress.
    NVAPI_WAIT_FOR_HW_RESOURCE = -222,  //!< Wait for HW resources allocation
    NVAPI_REQUIRE_FURTHER_HDCP_ACTION =
        -223,  //!< operation requires further HDCP action
    NVAPI_DISPLAY_MUX_TRANSITION_FAILED =
        -224,                             //!< Dynamic Mux transition failure
    ...
    NVAPI_FAILED_TO_LOAD_FROM_DRIVER_STORE =
        -228,  //!< There was an error while loading nvapi.dll from the driver
               //!< store.
    NVAPI_NO_VULKAN = -229,  //!< OpenGL does not export Vulkan fake extensions
    NVAPI_REQUEST_PENDING =
        -230,  //!< A request for NvTOPPs telemetry CData has already been made
               //!< and is pending a response.
    NVAPI_RESOURCE_IN_USE = -231,  //!< Operation cannot be performed because
    ...
    NVAPI_JIT_COMPILER_NOT_FOUND =
        -235,                     //!< PTX JIT compiler library was not found.
    NVAPI_INVALID_SOURCE = -236,  //!< Device kernel source is invalid.
    NVAPI_ILLEGAL_INSTRUCTION = -237,  //!< While executing a kernel, the device
                                       //!< encountered an illegal instruction.
    NVAPI_INVALID_PC = -238,  //!< While executing a kernel, the device program
                              //!< counter wrapped its address space
    NVAPI_LAUNCH_FAILED =
        -239,  //!< An exception occurred on the device while executing a kernel
    ...
    NVAPI_CALLBACK_NOT_FOUND =
        -242,  //!< The callback function is not found or not registered.
    NVAPI_INVALID_OUTPUT_WIRE_FORMAT =
        -243,  //!< Invalid Wire Format for the VR HMD
} NvAPI_Status;
...
typedef struct {
    ...
    NvU32 curAvailableDedicatedVideoMemory;  //!< Size(in kb) of the current
                                             ...
                                             //!< for allocating video memory
                                             //!< surfaces.
    NvU32 dedicatedVideoMemoryEvictionsSize;  //!< Size(in kb) of the total size
                                              //!< of memory released as a
                                              //!< result of the evictions.
    NvU32
        dedicatedVideoMemoryEvictionCount;  //!< Indicates the number of
                                            //!< eviction events that caused an
                                            //!< allocation to be removed from
                                            //!< dedicated video memory to free
                                            //!< GPU video memory to make room
...
#define NV_DISPLAY_DRIVER_MEMORY_INFO_VER_3 \
...
//!                If the GPU is in TCC Mode, only dedicatedVideoMemory will be
//!                returned in pMemoryInfo (NV_DISPLAY_DRIVER_MEMORY_INFO).
//!
//! \deprecated  Do not use this function - it is deprecated in release 520.
//! Instead, use NvAPI_GPU_GetMemoryInfoEx. SUPPORTED OS:  Windows 7 and higher
//!
//!
//! TCC_SUPPORTED
//!
...
__nvapi_deprecated_function(
    "Do not use this function - it is deprecated in release 520. Instead, use "
    "NvAPI_GPU_GetMemoryInfoEx.") NVAPI_INTERFACE
...
typedef struct {
    ...
    NvU64 dedicatedVideoMemory;  //!< Size(in bytes) of the physical
                                 //!< framebuffer. Refers to the dedicated video
                                 //!< memory on discrete GPUs.
                                 //!  It is more performant for GPU operations
                                 //!  than the reserved systemVideoMemory.
    NvU64 availableDedicatedVideoMemory;  //!< Size(in bytes) of the available
                                          //!< physical framebuffer for
                                          //!< allocating video memory surfaces.
    NvU64 systemVideoMemory;   //!< Size(in bytes) of system memory the driver
                               //!< allocates at load time. It is a substitute
                               //!< for dedicated video memory. Typically used
                               //!< with integrated GPUs that do not have
                               //!< dedicated video memory.
    NvU64 sharedSystemMemory;  //!< Size(in bytes) of shared system memory that
                               //!< driver is allowed to commit for surfaces
                               //!< across all allocations. On discrete GPUs, it
                               //!< is used to utilize system memory for various
                               //!< operations. It does not need to be reserved
                               //!< during boot. It may be used by both GPU and
                               //!< CPU, and has an “on-demand” type of usage.
    NvU64 curAvailableDedicatedVideoMemory;  //!< Size(in bytes) of the current
                                             //!< available physical framebuffer
                                             //!< for allocating video memory
                                             //!< surfaces.
    NvU64 dedicatedVideoMemoryEvictionsSize;  //!< Size(in bytes) of the total
                                              //!< size of memory released as a
                                              //!< result of the evictions.
    NvU64
        dedicatedVideoMemoryEvictionCount;  //!< Indicates the number of
                                            //!< eviction events that caused an
                                            ...
                                            //!< GPU video memory to make room
                                            //!< for other allocations.
    NvU64 dedicatedVideoMemoryPromotionsSize;  //!< Size(in bytes) of the total
                                               //!< size of memory allocated as
                                               //!< a result of the promotions.
    NvU64 dedicatedVideoMemoryPromotionCount;  //!< Indicates the number of
                                               //!< promotion events that caused
                                               //!< an allocation to be promoted
                                               //!< to dedicated video memory
...
#define NV_GPU_MEMORY_INFO_EX_VER_1 \
    MAKE_NVAPI_VERSION(NV_GPU_MEMORY_INFO_EX_V1, 1)
...
//!   for the specified GPU.
//!                If the GPU is in TCC Mode, only dedicatedVideoMemory will be
//!                returned in pMemoryInfo (NV_GPU_MEMORY_INFO_EX).
//!
...
//!  information is to be extracted.
//!  \param [out]  pMemoryInfo   The memory footprint available in the driver.
//!  See NV_GPU_MEMORY_INFO_EX.
//!
//!  \retval       NVAPI_INVALID_ARGUMENT             pMemoryInfo is NULL.
//!  \retval       NVAPI_OK                           Call successful.
//!  \retval       NVAPI_NVIDIA_DEVICE_NOT_FOUND      No NVIDIA GPU driving a
//!  display was found.
//!  \retval       NVAPI_INCOMPATIBLE_STRUCT_VERSION  NV_GPU_MEMORY_INFO_EX
//!  structure version mismatch.
//!
//!  \ingroup  driverapi
///////////////////////////////////////////////////////////////////////////////
NVAPI_INTERFACE NvAPI_GPU_GetMemoryInfoEx(
    NvPhysicalGpuHandle hPhysicalGpu,
...
//! \param [in]  pResource  The ID3D11Resource, ID3D10Resource or
