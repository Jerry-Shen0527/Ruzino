...
void NumLightsNodeSlang::createVariables(
    const ShaderNode&,
    GenContext&,
    Shader& shader) const
{
    ...
    ShaderPort* numActiveLights = addStageUniform(
        HW::PRIVATE_UNIFORMS,
        Type::INTEGER,
        HW::T_NUM_ACTIVE_LIGHT_SOURCES,
        ps);
    numActiveLights->setValue(Value::createValue<int>(0));
}

void NumLightsNodeSlang::emitFunctionDefinition(
    ...
    ShaderStage& stage) const
{
    DEFINE_SHADER_STAGE(stage, Stage::PIXEL)
    {
        ...
        shadergen.emitLine(
            "return min(" + HW::T_NUM_ACTIVE_LIGHT_SOURCES + ", " +
