...
#undef DECLARE_HAS_MEMBER
        ...
        struct FringeScaleRef {
            ...
            // Overload is present when ImDrawList does not have _FringeScale
            // member variable.
            template<typename T>
            static float& Get(
                typename std::enable_if<!HasFringeScale<T>::value, T>::type*)
        ...
        //------------------------------------------------------------------------------
        enum class ObjectType { None, Node, Link, Pin };
        ...
        struct Object {
            ...
            virtual bool TestHit(const ImRect& rect, bool allowIntersect = true)
                const
            {
                if (!m_IsLive)
                    return false;
                ...
                return !ImRect_IsEmpty(bounds) &&
                       (allowIntersect ? bounds.Overlaps(rect)
        ...
        enum class NodeType { Node, Group };
        ...
        struct Link final : Object {
            ...
            bool TestHit(const ImVec2& point, float extraThickness = 0.0f)
                const final;
            bool TestHit(const ImRect& rect, bool allowIntersect = true)
                const final;
        ...
        struct FlowAnimation final : Animation {
            ...
            void
            Flow(Link* link, float markerDistance, float speed, float duration);
        ...
        struct AnimationController {
            ...
            AnimationController(EditorContext* editor) : Editor(editor)
            {
            }
        ...
        struct NavigateAction final : EditorAction {
            enum class ZoomMode { None, Exact, WithMargin };
        ...
        struct CreateItemAction final : EditorAction {
            enum Stage { None, Possible, Create };

            enum Action { Unknown, UserReject, UserAccept };

            enum Type { NoItem, Node, Link };

            enum Result { True, False, Indeterminate };
        ...
        enum class SuspendFlags : uint8_t { None = 0, KeepSplitter = 1 };
        ...
        struct EditorContext {
            ...
            void NavigateTo(
                ...
                float duration = -1)
            {
                auto zoomMode = zoomIn ? NavigateAction::ZoomMode::WithMargin
