...
ImRect_ClosestPoint(const ImRect& rect, const ImVec2& p, bool snap_to_edge)
{
    ...
    return ImVec2(
        ...
        (p.y > rect.Max.y) ? rect.Max.y
                           : (p.y < rect.Min.y ? rect.Min.y : p.y));
}
...
inline ImVec2 ImRect_ClosestPoint(const ImRect& rect, const ImRect& other)
{
    ...
    if (other.Min.x >= rect.Max.x)
        ...
        result.x =
            (ImMax(rect.Min.x, other.Min.x) + ImMin(rect.Max.x, other.Max.x)) /
            2;
    ...
    if (other.Min.y >= rect.Max.y)
        ...
        result.y =
            (ImMax(rect.Min.y, other.Min.y) + ImMin(rect.Max.y, other.Max.y)) /
            2;
...
inline ImLine ImRect_ClosestLine(const ImRect& rect_a, const ImRect& rect_b)
{
    ...
    auto distribute =
        [](float& a, float& b, float a0, float a1, float b0, float b1) {
            if (a0 >= b1 || a1 <= b0)
                return;
