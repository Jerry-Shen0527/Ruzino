...
template<typename T>
inline T
ImCubicBezierSample(const T& p0, const T& p1, const T& p2, const T& p3, float t)
{
...
{
    // Legendre-Gauss abscissae with n=24 (x_i values, defined at i=n as the
    // roots of the nth order Legendre polynomial Pn(x))
    ...
    static_assert(
        sizeof(t_values) / sizeof(*t_values) ==
...
ImCubicBezierSplit(const T& p0, const T& p1, const T& p2, const T& p3, float t)
{
    ...
    return ImCubicBezierSplitResultT<T>{
        ImCubicBezierPointsT<T>{ p0,
                                 z1 * p1 - s1 * p0,
                                 z2 * p2 - 2 * z1 * s1 * p1 + s2 * p0,
                                 z3 * p3 - 3 * z2 * s1 * p2 + 3 * z1 * s2 * p1 -
                                     s3 * p0 },
...
#define IM_VEC2_INDEX(v, i) *(&v.x + i)
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        if (IM_VEC2_INDEX(delta_squared, i) >= 0) {
            ...
            auto t0 =
                (-IM_VEC2_INDEX(b, i) + delta) / (2 * IM_VEC2_INDEX(a, i));
            if (t0 > 0 && t0 < 1) {
                auto p = ImCubicBezier(
                    IM_VEC2_INDEX(p0, i),
                    IM_VEC2_INDEX(p1, i),
                    IM_VEC2_INDEX(p2, i),
                    IM_VEC2_INDEX(p3, i),
...
inline ImProjectResult ImProjectOnCubicBezier(
    ...
    const int subdivisions)
{
    return ImProjectOnCubicBezier(
        p, curve.P0, curve.P1, curve.P2, curve.P3, subdivisions);
}
...
{
    auto cubic_roots =
        [](float a, float b, float c, float d, float* roots) -> int {
        int count = 0;
        ...
        {
            auto S = sign(R + ImSqrt(D)) *
                     ImPow(ImFabs(R + ImSqrt(D)), (1.0f / 3.0f));
            auto T = sign(R - ImSqrt(D)) *
                     ImPow(ImFabs(R - ImSqrt(D)), (1.0f / 3.0f));
            ...
            auto Im =
                ImFabs(ImSqrt(3) * (S - T) / 2);  // complex part of root pair
...
inline ImCubicBezierIntersectResult ImCubicBezierLineIntersect(
    ...
    const ImLine& line)
{
    return ImCubicBezierLineIntersect(
        curve.P0, curve.P1, curve.P2, curve.P3, line.A, line.B);
}
...
{
    return ImCubicBezierSubdivide(
        callback,
        user_pointer,
        ImCubicBezierPoints{ p0, p1, p2, p3 },
        tess_tol,
        flags);
}
    ...
    ImCubicBezierSubdivideFlags flags)
{
    struct Tesselator {
        ...
        void Subdivide(const ImCubicBezierPoints& curve, int level = 0)
        {
            ...
            float d2 =
                ((curve.P1.x - curve.P3.x) * dy -
                 (curve.P1.y - curve.P3.y) * dx);
            float d3 =
                ((curve.P2.x - curve.P3.x) * dy -
                 (curve.P2.y - curve.P3.y) * dx);
            d2 = (d2 >= 0) ? d2 : -d2;
            d3 = (d3 >= 0) ? d3 : -d3;
            if ((d2 + d3) * (d2 + d3) <
                TesselationTollerance * (dx * dx + dy * dy)) {
                ...
                Subdivide(
                    ImCubicBezierPoints{ curve.P0, p12, p123, p1234 },
                    level + 1);
                Subdivide(
                    ImCubicBezierPoints{ p1234, p234, p34, curve.P3 },
                    level + 1);
            }
        }
    };
...
inline void ImCubicBezierSubdivide(
    ...
    ImCubicBezierSubdivideFlags flags)
{
    auto handler = [](const ImCubicBezierSubdivideSample& p,
                      void* user_pointer) {
        auto& callback = *reinterpret_cast<F*>(user_pointer);
        callback(p);
    };

    ImCubicBezierSubdivide(
        handler,
        &callback,
        ImCubicBezierPoints{ p0, p1, p2, p3 },
        tess_tol,
...
inline void ImCubicBezierFixedStep(
    ...
    float max_t_error)
{
    auto handler = [](ImCubicBezierFixedStepSample& sample,
                      void* user_pointer) {
        auto& callback = *reinterpret_cast<F*>(user_pointer);
        callback(sample);
    };
