...
// Phase bodies are defined below; the merged execute chains them in the
// paper's order over ONE shared state.
static bool wb_sim_phase_deposit(
    ExeParams params,
    Ruzino::WetbrushZoneState& zs_in);
static bool wb_sim_phase_bristle(
    ExeParams params,
    Ruzino::WetbrushZoneState& zs_in);
static bool wb_sim_phase_fluid(
    ExeParams params,
    Ruzino::WetbrushZoneState& zs_in);
...
NODE_EXECUTION_FUNCTION(brush_wb_sim)
{
    ...
    if (!wb_sim_phase_deposit(params, zs))
        return false;
    if (!wb_sim_phase_bristle(params, zs))
        return false;
    if (!wb_sim_phase_fluid(params, zs))
        return false;
...
// ======================================================================
// PHASE 1 — brush & canvas preparation (paper §4.1/§4.2; ex-node
// brush_wb_deposit): lazy allocation, active-window follow/scroll, dip,
// per-substep bristle dynamics -> psi/velocity-BC raster.
// ======================================================================
static bool wb_sim_phase_deposit(
    ExeParams params,
    Ruzino::WetbrushZoneState& zs_in)
{
    ...
    // zs arrives from the merged node's execute: the optional sim_in
    // feedback, absent on the init frame — the allocation below fills it,
    // and phases 2/3 must see that allocation (hence the reference).
    Ruzino::WetbrushZoneState& zs = zs_in;
    ...
    if (!payload.is_simulating) {
        return true;  // end PHASE 1 only — later phases keep their own gates
    }
...
// brush_wb_bristle): ABSORB (grid paint -> sample, Eq.12/13 capacity) then
// EMIT (over-capacity sample -> FLIP particles).
// ======================================================================
static bool wb_sim_phase_bristle(
    ExeParams params,
    Ruzino::WetbrushZoneState& zs_in)
{
    ...
    // stay well inside D1 so newborns ride the adhesion bulb; with ρ₀=6.0,
    // M_max=0.15 → R_j≈0.18 cm ≈ 0.36×brush_radius (paper ratio ~0.35).
    // M_max and ρ₀ move TOGETHER (M_max = dip size — how long a stroke one
    // dip paints; ρ₀ keeps R_j constant): the pair (0.03, 2e4) drained a dip
    // in ~15 frames at the old 1u=27.5cm scale; (0.15, 1e5) carried 5× the
    // paint at the same R_j. In the current 1u=1cm scale the pair is
    // (0.15, 6.0) — same R_j ≈ 0.18 cm. Larger M_max without raising ρ₀
    // scatters emitted particles past the brush (wide diffuse blob instead of
    // a stroke). Also the dip load (m_j = M_max at stroke_start) — shared
    // constant.
    ...
    Ruzino::WetbrushZoneState& zs_in)
{
    ...
    // Grid↔particle conversion + adhesion ranges (paper §5.2 Table 1). WORLD
    // SCALE 1 unit = 1 cm (doc §33), so the paper's SI values apply VERBATIM:
    // D0 = 1 cm, D1 = 0.3 cm — and our brush (radius 0.5 cm ≈ the paper's
    // ~0.55 cm brush) makes the paper's own absolute numbers the right
    // calibration, replacing the earlier brush-relative inference
    // (D0 = 1.8 R, D1 = 0.55 R). Note the ride time grows (~D0 shell width /
    ...
    // the grid liquid previously had NO force and hovered where deposited).
    // Direction-adjustable for experiments via WB_GRAVITY_X/Y/Z.
    ...
    ensure_prog(field->advect_scalar_program, "fluid_advect_upwind.slang");
    ...
    if (sim_dt > 1e-6f) {
        ...
        for (int s = 0; s < substeps; s++) {
            ...
            // dt·ν·N² silently assumed a unit-size domain (h = 1/N) and held
            // only while paper_size was 1. The 1u=1cm rescale (paper_size=10)
            // overstated ν by paper² = 100×: a ≈ 5.5e3 made each sweep an
            // almost-exact window mean, dragging the wake at brush speed —
            // deposits flickered (Eq.15 drained the trail into particles that
            // inherited the homogenized velocity and never satisfied the
            // slow-deposit gate) and the per-frame window scroll left sawtooth
            // scales. Viscosity is now a kinematic viscosity in cm²/s.
