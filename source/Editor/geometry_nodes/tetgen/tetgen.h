...
class tetgenbehavior {
   public:
    ...
    tetgenbehavior()
    {
        ...
        smooth_cirterion =
            3;               // -s# default smooth surface and volume vertices.
...
class tetgenmesh {
   public:
    ...
    // // contains a 2D subcomplex S which is a triangular mesh of the boundary
    // of   // X. S contains a 1D subcomplex L which is a linear mesh of the
    // boundary of  // S. Faces and edges in S and L are respectively called
    // subfaces and segme-  // nts to distinguish them from others in T. //
    //                                                                            //
    // TetGen uses a tetrahedron-based data structure.  It stores tetrahedra and
    // // vertices.  This data structure is pointer-based. Each tetrahedron
    // contains // pointers to its vertices and adjacent tetrahedra. Each vertex
    // holds its x-,// y-, z-coordinates, and a pointer to one of the tetrahedra
    // having it.  Both // tetrahedra and vertices may contain user data. //
    //                                                                            //
    // Let T be a tetrahedralization. Each triangular face of T belongs to
    // either // two or one tetrahedron.  In the latter case, it is an exterior
    // boundary    // face of T. TetGen attaches tetrahedra (one-to-one) to such
    // faces. All such // tetrahedra contain an "infinite vertex" (which has no
    // geometric coordinates//
    // ).  One can imagine such a vertex lies in 4D space and is visible by all
    // // exterior boundary faces simultaneously.  This extended set of
    // tetrahedra   // (including the infinite vertex) becomes a
    // tetrahedralization of a 3-sphere // that has no boundary in 3d. It has
    // the nice property that every triangular // face is shared by exactly two
    // tetrahedra.                                  //
    //                                                                            //
    ...
    // // operating on structures referred as ``handles". A handle is a pair
    // (t,v),  // where t is a pointer to a tetrahedron, and v is a 4-bit
    // integer, in the    // range from 0 to 11. v is called the ``version'' of
    // a tetrahedron, it rep-  // resents a directed edge of a specific face of
    // the tetrahedron.             //
    //                                                                            //
    ...
    // versions  // can be grouped into 4 distinct ``edge rings'' in 4
    // ``oriented faces'' of   // this tetrahedron.  One can encode each version
    // (a directed edge) into a    // 4-bit integer such that the two upper bits
    // encode the index (from 0 to 2)  // of this edge in the edge ring, and the
    // two lower bits encode the index (   // from 0 to 3) of the oriented face
    // which contains this edge.                //
    //                                                                            //
    // The four vertices of a tetrahedron are indexed from 0 to 3 (according to
    // // their storage in the data structure).  Give each face the same index
    // as    // the node opposite it in the tetrahedron.  Denote the edge
    // connecting face  // i to face j as i/j. We number the twelve versions as
    // follows:              //
    //                                                                            //
    ...
    //    6 (2/1)   10 (2/0)                                // face 3 |   3
    //    (3/0)    7 (3/1)   11 (3/2)                                //
    //                                                                            //
    // Similarly, navigation and manipulation in a (boundary) triangulation are
    // // done by using handles of triangles. Each handle is a pair (s, v),
    // where s  // is a pointer to a triangle, and v is a version in the range
    // from 0 to 5.   // Each version corresponds to a directed edge of this
    // triangle.              //
    //                                                                            //
    // Number the three vertices of a triangle from 0 to 2 (according to their
    // // storage in the data structure). Give each edge the same index as the
    // node  // opposite it in the triangle. The six versions of a triangle are:
    // //
    //                                                                            //
    //                 | edge 0   edge 1   edge 2 //
    //  ---------------|-------------------------- //
    ...
    // // is the base-2 logarithm of 'objectsperblock'; 'objects' counts the
    // number  // of allocated objects; 'totalmemory' is the total memory in
    // bytes.          //
    //                                                                            //
    //============================================================================//
...
#define fastlookup(pool, index)                                        \
    ...
    //   // of free memory for an item. deaditemstack is the head of a linked
    //   list   // (stack) of deallocated items that can be recycled.
    //   unallocateditems is  // the number of items that remain to be allocated
    //   from nowblock.           //
    //                                                                            //
    // Traversal is the process of walking through the entire list of items, and
    // //
    //   is separate from allocation.  Note that a traversal will visit items on
    //   // the "deaditemstack" stack as well as live items.  pathblock points
    //   to    // the block currently being traversed.  pathitem points to the
    //   next item   // to be traversed.  pathitemsleft is the number of items
    //   that remain to    // be traversed in pathblock. //
    //                                                                            //
    //============================================================================//
    ...
    class flipconstraints {
       public:
        ...
        REAL max_asp_out;   // Max asp ratio after the improvement of dihedral
                            // angle.
    ...
    int* idx_segment_ridge_vertex_list;  // are two ridge vertices form a
                                         // segment?
    ...
    // scheme from Edelsbrunner, et al [*].  Hence the point-in-sphere test
    // never // returns a zero. The idea is to perturb the weights of vertices
    // in 4D.      //
    //                                                                            //
    // The routine tri_edge_test() determines whether or not a triangle and an
    // // edge intersect in 3D. If they do cross, their intersection type is
    // also    // reported. This test is a combination of n 3D orientation tests
    // (3 < n < 9).// It uses the robust orient3d() test to make the branch
    // decisions.           //
    //                                                                            //
    ...
    REAL
    orient4d_s(REAL*, REAL*, REAL*, REAL*, REAL*, REAL, REAL, REAL, REAL, REAL);
    ...
    int tri_edge_tail(
        ...
        int*);
    int
    tri_edge_test(point, point, point, point, point, point, int, int*, int*);
    ...
    void
    facenormal(point pa, point pb, point pc, REAL* n, int pivot, REAL* lav);
    ...
    // // flips performed within the convex hull of five vertices: 2-to-3,
    // 3-to-2,   // 1-to-4, and 4-to-1 flips. The numbers indicate the number of
    // tetrahedra    // before and after each flip.  The 1-to-4 and 4-to-1 flip
    // involve inserting  // or deleting a vertex, respectively. //
    //                                                                            //
    // There are complex local transformations that are a combination of
    // element- // ary flips. For example, a 4-to-4 flip, which replaces two
    // coplanar edges,  // combines a 2-to-3 flip and a 3-to-2 flip. Note that
    // the first 2-to-3 flip  // will temporarily create a degenerate
    // tetrahedron removed immediately by    // the followed 3-to-2 flip.  More
    // generally, an n-to-m flip, where n > 3,    // m = (n - 2) * 2, which
    // removes an edge, can be done by first performing a  // sequence of (n -
    // 3) 2-to-3 flips followed by a 3-to-2 flip.                //
    //                                                                            //
    // The routines flip23(), flip32(), and flip41() perform the three
    // elementray // flips. The flip14() is available inside the routine
    // insertpoint().         //
    //                                                                            //
    ...
    // // for constructing Delaunay tetrahedralizations (DTs):  the
    // Bowyer-Watson    // (B-W) algorithm and the incremental flip algorithm of
    // Edelsbrunner and     // Shah, "Incremental topological flipping works for
    // regular triangulation,"  // Algorithmica, 15:233-241, 1996. //
    //                                                                            //
    // The routine incrementalflip() implements the flip algorithm of
    // [Edelsbrun- // ner and Shah, 1996].  It flips a queue of locally
    // non-Delaunay faces (in   // arbitrary order).  The success is guaranteed
    // when the Delaunay tetrahedra- // lization is constructed incrementally by
    // adding one vertex at a time.      //
    //                                                                            //
    // The routine locate() finds a tetrahedron contains a new point in current
    // // DT. It uses a simple stochastic walk algorithm: starting from an
    // arbitrary // tetrahedron in DT, it finds the destination by visit one
    // tetrahedron at a  // time, randomly chooses a tetrahedron if there are
    // more than one choices.   // This algorithm terminates due to
    // Edelsbrunner's acyclic theorem.           //
    //                                                                            //
    // Choose a good starting tetrahedron is crucial to the speed of the walk.
    // // TetGen initially uses the "jump-and-walk" algorithm of Muecke, E.P.,
    // Saias,// I., and Zhu, B. "Fast Randomized Point Location Without
    // Preprocessing." In // Proceedings of the 12th ACM Symposium on
    // Computational Geometry, 274-283,  // 1996.  It first randomly samples
    // several tetrahedra in the DT and then     // choosing the closet one to
    // start walking.                                  //
    //                                                                            //
    ...
    // Computa- // tional Geometry, 211-219, 2003. On the other hand, Liu and
    // Snoeyink showed // that the point location could be made in constant time
    // if the points are   // pre-sorted so that the nearby points in space have
    // nearby indices, then    // adding the points in this order. They sorted
    // the points along the 3D       // Hilbert curve. //
    //                                                                            //
    ...
    // The Hilbert   // indices is calculated by Butz's algorithm in 1971. An
    // excellent exposition // of this algorithm can be found in the paper of
    // Hamilton, C., "Compact      // Hilbert Indices", Technical Report
    // CS-2006-07, Computer Science, Dalhousie // University, 2006 (the Section
    // 2). My implementation also referenced Steven // Witham's performance of
    // "Hilbert walk" (hopefully, it is still available   // at
    // http://www.tiac.net/~sw/2008/10/Hilbert/). //
    //                                                                            //
    // TetGen sorts the points using the method in the paper of
    // Boissonnat,J.-D., // Devillers, O. and Hornus, S. "Incremental
    // Construction of the Delaunay     // Triangulation and the Delaunay Graph
    // in Medium Dimension," In Proceedings  // of the 25th ACM Symposium on
    // Computational Geometry, 2009.  It first       // randomly sorts the
    // points into subgroups using the Biased Randomized       // Insertion
    // Ordering (BRIO) of Amenta et al 2003, then sorts the points in   // each
    // subgroup along the 3D Hilbert curve.  Inserting points in this order  //
    ...
    void hilbert_sort3(
        ...
        int depth);
    void
    brio_multiscale_sort(point*, int, int threshold, REAL ratio, int* depth);

    // Point location.
    unsigned long randomnation(unsigned int choices);
    void randomsample(point searchpt, triface* searchtet);
    enum locateresult
    locate(point searchpt, triface* searchtet, int chkencflag = 0);
    ...
    //                                                                            //
    // A constrained Delaunay tetrahedralization (CDT) is a variation of a
    // Delau- // nay tetrahedralization (DT) that respects the boundary of a 3D
    // PLC (mesh   // domain).  A crucial difference between a CDT and a DT is
    // that triangles in // the PLC's polygons are not required to be locally
    // Delaunay, which frees    // the CDT to respect the PLC's polygons better.
    // CDTs have optimal properties // similar to those of DTs. //
    //                                                                            //
    // Steiner Points and Steiner CDTs. It is well-known that even a simple 3D
    // // polyhedron may not have a tetrahedralization which only uses its
    // vertices. // Some extra points, so-called "Steiner points" are needed to
    // form a tetrah- // edralization of such polyhedron.  A Steiner CDT of a 3D
    // PLC is a CDT       // containing Steiner points. TetGen generates Steiner
    // CDTs.                  //
    //                                                                            //
    // The routine constraineddelaunay() creates a (Steiner) CDT of the PLC //
    // (including Steiner points). It has two steps, (1) segment recovery and
    // (2) // facet (polygon) recovery. //
    //                                                                            //
    ...
    // // into non-Delaunay segments until all subsegments appear together in a
    // DT.  // The running time of this algorithm is proportional to the number
    // of        // Steiner points. //
    //                                                                            //
    ...
    // // and Constrained Regular Triangulations by Flips." In Proceedings of
    // the    // 19th ACM Symposium on Computational Geometry, 86-95, 2003. //
    //                                                                            //
    // Although no Steiner point is needed in step (2), a facet with
    // non-coplanar // vertices might need Steiner points. It is discussed in
    // the paper of Si, H.,// and  Shewchuk, J., "Incrementally Constructing and
    // Updating Constrained    // Delaunay Tetrahedralizations with Finite
    // Precision Coordinates." In        // Proceedings of the 21th
    // International Meshing Roundtable, 2012.            //
    //                                                                            //
    // Our implementation of the facet recovery algorithms recovers a "missing
    // // region" at a time. Each missing region is a subset of connected
    // interiors  // of a polygon. The routine formcavity() creates the cavity
    // of crossing      // tetrahedra of the missing region. The cavity
    // re-triangulation algorithm is // implemented by three subroutines,
    // delaunizecavity(), fillcavity(), and     // carvecavity(). Since it may
    // fail due to non-coplanar vertices, the         // subroutine
    // restorecavity() is used to restore the original cavity.         //
    //                                                                            //
    ...
    // fails to recover a missing region. It inserts Steiner points to refine
    // the // missing region. To avoid inserting Steiner points very close to
    // existing   // segments.  The classical encroachment rules of the Delaunay
    // refinement     // algorithm are used to choose the Steiner points.  The
    // routine              // constrainedfacets() does the facet recovery by
    // using either the cavity re- // triangulation algorithm (default) or the
    // flip algorithm. It results in a   // CDT of the (modified) PLC (including
    // Steiner points).                      //
    //                                                                            //
    //============================================================================//
    ...
    enum interresult
    scoutsegment(point, point, face*, triface*, point*, arraypool*);
    ...
    // // -shaped tetrahedra and appropriate mesh size.  It is necessary to
    // insert   // new Steiner points to achieve this property. The questions
    // are (1) how to  // choose the Steiner points? and (2) how to insert them?
    // //
    //                                                                            //
    ...
    // // algorithm guarantees that the mesh is size-optimal (to within a
    // constant   // factor) among all meshes with the same quality. //
    ...
    // // Mesh Generation by Delaunay Refinement," In Proceedings of the 14th
    // ACM    // Symposium on Computational Geometry, 86-95, 1998.  It
    // guarantees that all  // tetrahedra of the output mesh have a "radius-edge
    // ratio" (equivalent to    // the minimal face angle) bounded. However, it
    // does not remove slivers, a    // type of very flat tetrahedra which can
    // have no small face angles but have  // very small (and large) dihedral
    // angles. Moreover, it may not terminate if  // the input PLC contains
    // "sharp features", e.g., two edges (or two facets)   // meet at an acute
    // angle (or dihedral angle).                                //
    //                                                                            //
    // TetGen uses the basic Delaunay refinement scheme to insert Steiner
    // points. // While it always maintains a constrained Delaunay mesh.  The
    // algorithm is   // described in Si, H., "Adaptive Constrained Delaunay
    // Mesh Generation,"      // International Journal for Numerical Methods in
    // Engineering, 75:856-880.    // This algorithm always terminates and sharp
    // features are easily preserved.  // The mesh has good quality (same as
    // Shewchuk's Delaunay refinement algori-  // thm) in the bulk of the mesh
    // domain. Moreover, it supports the generation  // of adaptive mesh
    // according to a (isotropic) mesh sizing function.          //
    //                                                                            //
    //============================================================================//
    ...
    bool
    check_enc_subface(face* chkfac, point* pencpt, REAL* ccent, REAL* radius);
    ...
    enum locateresult
    locate_point_walk(point searchpt, triface*, int chkencflag);
...
inline void terminatetetgen(tetgenmesh* m, int x)
{
    ...
    switch (x) {
        ...
        case 3:
            printf(
                "The input surface mesh contain self-intersections. Program "
                "stopped.\n");
            // printf("Hint: use -d option to detect all
            // self-intersections.\n");
            break;
        case 4:
            printf(
                "A very small input feature size was detected. Program "
                "stopped.\n");
            if (m) {
                printf(
                    "Hint: use -T option to set a smaller tolerance. Current "
                    "is %g\n",
                    m->b->epsilon);
            }
            break;
        case 5:
            printf(
                "Two very close input facets were detected. Program "
                "stopped.\n");
            printf(
                "Hint: use -Y option to avoid adding Steiner points in "
                "boundary.\n");
            break;
        case 10:
        ...
        case 200:
            printf(
                "Boundary contains Steiner points (-YY option). Program "
                "stopped.\n");
            break;
    }  // switch (x)
    exit(x);
...
inline void
tetgenmesh::setelemattribute(tetrahedron* ptr, int attnum, REAL value)
{
...
inline tetgenmesh::point tetgenmesh::farsdest(face& s)
{
    ...
    while (1) {
        ...
        if (neighsh.sh == NULL)
            break;
        if (sdest(neighsh) != sdest(travesh))
            sesymself(neighsh);
