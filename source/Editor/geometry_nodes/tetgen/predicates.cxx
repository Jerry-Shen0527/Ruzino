...
#define Four_Four_Sum(                                              \
    a3, a2, a1, a0, b4, b3, b1, b0, x7, x6, x5, x4, x3, x2, x1, x0) \
    Four_Two_Sum(a3, a2, a1, a0, b1, b0, _l, _2, _1, _0, x1, x0);   \
    Four_Two_Sum(_l, _2, _1, _0, b4, b3, x7, x6, x5, x4, x3, x2)

#define Eight_One_Sum(                                                     \
    a7, a6, a5, a4, a3, a2, a1, a0, b, x8, x7, x6, x5, x4, x3, x2, x1, x0) \
    ...
    REAL maxz)
{
...
#ifdef SINGLE
    _control87(
        _PC_24, _MCW_PC); /* Set FPU control word for single precision. */
#else                     /* not SINGLE */
    _control87(
        _PC_53, _MCW_PC); /* Set FPU control word for double precision. */
#endif                    /* not SINGLE */
#endif                    /* CPU86 */
#ifdef LINUX
#ifdef SINGLE
    ...
    ispstaticfilter =
        1.2466136531027298e-13 * maxx * maxy * maxz * (maxz * maxz);
}
...
/*                                                                           */
/*****************************************************************************/
static int scale_expansion(int elen, REAL* e, REAL b, REAL* h)
...
REAL orient2dexact(REAL* pa, REAL* pb, REAL* pc)
{
    ...
    Two_Two_Diff(
        axby1, axby0, axcy1, axcy0, aterms3, aterms[2], aterms[1], aterms[0]);
    aterms[3] = aterms3;

    Two_Product(pb[0], pc[1], bxcy1, bxcy0);
    Two_Product(pb[0], pa[1], bxay1, bxay0);
    Two_Two_Diff(
        bxcy1, bxcy0, bxay1, bxay0, bterms3, bterms[2], bterms[1], bterms[0]);
    bterms[3] = bterms3;

    Two_Product(pc[0], pa[1], cxay1, cxay0);
    Two_Product(pc[0], pb[1], cxby1, cxby0);
    Two_Two_Diff(
        cxay1, cxay0, cxby1, cxby0, cterms3, cterms[2], cterms[1], cterms[0]);
    cterms[3] = cterms3;
...
REAL orient3dslow(REAL* pa, REAL* pb, REAL* pc, REAL* pd)
{
    ...
    alen = fast_expansion_sum_zeroelim(
        temp32len, temp32, temp32tlen, temp32t, adet);
    ...
    blen = fast_expansion_sum_zeroelim(
        temp32len, temp32, temp32tlen, temp32t, bdet);
    ...
    clen = fast_expansion_sum_zeroelim(
        temp32len, temp32, temp32tlen, temp32t, cdet);
...
{
    return (REAL)-cgal_pred_obj.orientation_3_object()(
...
REAL insphereexact(REAL* pa, REAL* pb, REAL* pc, REAL* pd, REAL* pe)
{
    ...
    bcdelen = fast_expansion_sum_zeroelim(
        temp48alen, temp48a, temp48blen, temp48b, bcde);
    ...
    cdealen = fast_expansion_sum_zeroelim(
        temp48alen, temp48a, temp48blen, temp48b, cdea);
    ...
    deablen = fast_expansion_sum_zeroelim(
        temp48alen, temp48a, temp48blen, temp48b, deab);
    ...
    eabclen = fast_expansion_sum_zeroelim(
        temp48alen, temp48a, temp48blen, temp48b, eabc);
    ...
    abcdlen = fast_expansion_sum_zeroelim(
        temp48alen, temp48a, temp48blen, temp48b, abcd);
...
REAL insphereslow(REAL* pa, REAL* pb, REAL* pc, REAL* pd, REAL* pe)
{
    ...
    temp64alen = fast_expansion_sum_zeroelim(
        temp32alen, temp32a, temp32blen, temp32b, temp64a);
    ...
    temp64clen = fast_expansion_sum_zeroelim(
        temp32alen, temp32a, temp32blen, temp32b, temp64c);
    temp128len = fast_expansion_sum_zeroelim(
        temp64alen, temp64a, temp64blen, temp64b, temp128);
    temp192len = fast_expansion_sum_zeroelim(
        temp64clen, temp64c, temp128len, temp128, temp192);
...
{
    return (REAL)-cgal_pred_obj.side_of_oriented_sphere_3_object()(
        Point(pa[0], pa[1], pa[2]),
        Point(pb[0], pb[1], pb[2]),
        Point(pc[0], pc[1], pc[2]),
        Point(pd[0], pd[1], pd[2]),
    ...
    REAL permanent)
{
    ...
    aceps = (aex * ceytail + cey * aextail) - (aey * cextail + cex * aeytail);
    bdeps = (bex * deytail + dey * bextail) - (bey * dextail + dex * beytail);
    det += ((beheight * ((cez * daeps + dez * aceps + aez * cdeps) +
                         (ceztail * da3 + deztail * ac3 + aeztail * cd3)) +
             deheight * ((aez * bceps - bez * aceps + cez * abeps) +
                         (aeztail * bc3 - beztail * ac3 + ceztail * ab3))) -
            (aeheight * ((bez * cdeps - cez * bdeps + dez * bceps) +
                         (beztail * cd3 - ceztail * bd3 + deztail * bc3)) +
             ceheight * ((dez * abeps + aez * bdeps + bez * daeps) +
                         (deztail * ab3 + aeztail * bd3 + beztail * da3)))) +
