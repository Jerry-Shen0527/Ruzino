...
#include "brush_sim_common.hpp"  // WetbrushState, SimConstants, helpers (shared)
...
namespace {
    ...
    void write_stroke_record(const StrokeRecord& rec)
    {
        ...
        spdlog::info(
            ...
            rec.points.size(),
            rec.stroke_lengths.size(),
            stroke_recorder_path());
    }

    // NOTE: buffer factories (create_field_buffer, create_typed_buffer,
    // create_byte_buffer), shader compile/dispatch helpers (compile_shader,
    // dispatch_field, upload_constant_buffer, reset_counter), and the
    // shader-constant structs (SimConstants, BristleConstants,
    // ParticleConstants, BristleLiquidConstants, ConstraintModeCB) now live
    // in brush_sim_common.h, shared with node_brush_wetbrush_step.cpp.
    // Short local aliases preserve the original call sites below unchanged.
    inline nvrhi::BufferHandle
    create_field_buffer(ResourceAllocator& rc, int n, const char* debug_name)
    {
        return brush_create_field_buffer(rc, n, debug_name);
    }
    inline nvrhi::BufferHandle create_typed_buffer(
        ResourceAllocator& rc, int count, int stride, const char* debug_name)
    {
        return brush_create_typed_buffer(rc, count, stride, debug_name);
    }
    inline nvrhi::BufferHandle create_byte_buffer(
        ResourceAllocator& rc, int size_bytes, const char* debug_name)
    {
        return brush_create_byte_buffer(rc, size_bytes, debug_name);
    }
    inline ProgramHandle
    compile_shader(ResourceAllocator& rc, const std::string& filename)
    {
        return brush_compile_shader(rc, filename);
    }
    inline void dispatch_field(
    ...
    {
        brush_dispatch(rc, prog, srvs, uavs, cb, total_threads);
    }
    inline void upload_constant_buffer(
        ResourceAllocator& rc, nvrhi::IDevice* device, const void* data,
        size_t size, const char* debug_name, nvrhi::BufferHandle& out_buf)
    {
        brush_upload_cb(rc, device, data, size, debug_name, out_buf);
    }
    inline void reset_counter(
        ResourceAllocator& rc, nvrhi::IDevice* device,
        nvrhi::BufferHandle& counter_buf)
    {
        brush_reset_counter(rc, device, counter_buf);
    }
...
// Shader-constant structs (SimConstants, BristleConstants,
// ParticleConstants, BristleLiquidConstants, ConstraintModeCB) and the
// simulation state struct (PaintSimStorage == WetbrushState) now live in
// brush_sim_common.h, shared with node_brush_wetbrush_step.cpp. The header
// provides `using PaintSimStorage = WetbrushState;` so all call sites below
// (get_storage<PaintSimStorage&>, PaintSimStorage::WIN_ALLOC_XY, etc.)
// resolve unchanged.
...
NODE_DECLARATION_FUNCTION(brush_paint_sim)
{
    ...
    b.add_output<float>(
        "Max Divergence");  // max|div u| in the active window (post-projection)
    b.add_output<float>(
        "Mean Divergence");  // mean |div u| in the active window
    b.add_output<float>(
        "Total Density");  // sum of grid density (paint mass proxy)
    ...
    b.add_output<float>(
        "Total Particle Mass");  // sum of alive particle mass (color.w)
}
...
NODE_EXECUTION_FUNCTION(brush_paint_sim)
{
    ...
    // separate curve primitives, all flattened into `vertices` with
    // `vert_counts` delimiting each. When a new stroke begins, the brush
    // "lifts" and "lands" elsewhere — there must be NO paint continuity (no
    // sub-step interpolation, no velocity inheritance) across that boundary,
    // otherwise the tail of one stroke gets connected to the head of the next
    // ("ghost line" symptom).
    auto vertex_is_stroke_start = [&](int vi) -> bool {
        if (vert_counts.empty())
            return vi == 0;  // single-curve fallback
        ...
        for (int c : vert_counts) {
            if (vi == acc)
                return true;  // first vertex of this primitive
            acc += c;
            if (vi < acc)
                return false;  // interior vertex of this primitive
        }
        return false;
    };
    ...
    // strokes outside it are simply clamped at deposit time (see
    // brush_deposit). Canvas Z = the bottom face of the box = the paper
    // surface. Paint stacks only upward (z >= 0 in grid space), giving a
    // single-sided paint layer instead of the previous z-mirror image across
    // the stroke plane.
    if (!storage.center_initialized) {
        ...
        float height = canvas_height_input > 1e-6f
                           ...
                           : paper_size * static_cast<float>(resolution_z) /
                                 static_cast<float>(
                                     resolution);  // auto: isotropic Z cells
        ...
        spdlog::info(
            "brush_paint_sim: grid {}x{}x{}, paper={:.3f}, height={:.3f}, "
            "cell={:.5f}, "
            "canvas_z={:.3f}",
            resolution,
            resolution,
    ...
    if (storage.grid_alloc_res != storage.grid_res ||
        ...
        safe_destroy_buf(storage.density);
        safe_destroy_buf(storage.density_tmp);
        safe_destroy_buf(storage.color_r);
        safe_destroy_buf(storage.color_y);
        safe_destroy_buf(storage.color_b);
        safe_destroy_buf(storage.color_tmp);
        safe_destroy_buf(storage.vel_x);
        safe_destroy_buf(storage.vel_x_tmp);
        safe_destroy_buf(storage.vel_y);
        safe_destroy_buf(storage.vel_y_tmp);
        safe_destroy_buf(storage.vel_z);
        safe_destroy_buf(storage.vel_z_tmp);
        safe_destroy_buf(storage.wetness);
        safe_destroy_buf(storage.wetness_tmp);
        safe_destroy_buf(storage.oil_density);
        safe_destroy_buf(storage.oil_density_tmp);
        safe_destroy_buf(storage.height_field);
        safe_destroy_buf(storage.pressure_a);
        safe_destroy_buf(storage.pressure_b);
        safe_destroy_buf(storage.divergence_buf);
        safe_destroy_buf(storage.bristle_density);
        safe_destroy_buf(storage.bristle_vel_x);
        safe_destroy_buf(storage.bristle_vel_y);
        safe_destroy_buf(storage.bristle_vel_z);
        safe_destroy_buf(storage.bristle_color_r);
        safe_destroy_buf(storage.bristle_color_y);
        safe_destroy_buf(storage.bristle_color_b);
        safe_destroy_buf(storage.ptcl_density);
        safe_destroy_buf(storage.ptcl_vel_x);
        safe_destroy_buf(storage.ptcl_vel_y);
        safe_destroy_buf(storage.ptcl_vel_z);
        safe_destroy_buf(storage.ptcl_rast_r);
        safe_destroy_buf(storage.ptcl_rast_y);
        safe_destroy_buf(storage.ptcl_rast_b);
        ...
        for (auto* buf : { &storage.density,         &storage.density_tmp,
                           &storage.color_r,         &storage.color_y,
                           &storage.color_b,         &storage.color_tmp,
                           &storage.vel_x,           &storage.vel_x_tmp,
                           ...
                           &storage.height_field,    &storage.pressure_a,
                           &storage.pressure_b,      &storage.divergence_buf,
                           ...
                           &storage.bristle_color_b, &storage.ptcl_density,
                           &storage.ptcl_vel_x,      &storage.ptcl_vel_y,
                           &storage.ptcl_vel_z,      &storage.ptcl_rast_r,
                           &storage.ptcl_rast_y,     &storage.ptcl_rast_b,
        ...
        for (auto* buf : { &storage.canvas_density,
                           &storage.canvas_color_r,
                           &storage.canvas_color_y,
                           &storage.canvas_color_b,
    ...
    if (!storage.bristles_initialized) {
        ...
        storage.bristle_data = create_typed_buffer(
            rc,
            Nb * M,
            PaintSimStorage::BRISTLE_VERTEX_STRIDE,
            "bristle_data");  // 2×float4 (pos3+vel3)
        storage.lambda_buf =
            create_typed_buffer(rc, Nb * M, sizeof(float), "lambda_buf");
        storage.sample_pos = create_typed_buffer(
            rc,
            Nb * S,
            sizeof(float) * 4,
            "sample_pos");  // float4 (.xyz + pad)
        storage.sample_vel = create_typed_buffer(
            rc,
            Nb * S,
            sizeof(float) * 4,
            "sample_vel");  // float4 (.xyz + pad)
        storage.sample_color =
            create_typed_buffer(rc, Nb * S, sizeof(float) * 4, "sample_color");
        // Bishop frame: SampleFrame { tangent, normal, binormal } — 3 float4 =
        // 48 bytes
        storage.sample_frame = create_typed_buffer(
            rc, Nb * S, sizeof(float) * 4 * 3, "sample_frame");

        // Bristle liquid state: SampleLiquid {float mass, float3 pigment} per
        // sample
        storage.sample_liquid =
            create_typed_buffer(rc, Nb * S, sizeof(float) * 4, "sample_liquid");
        ...
        storage.sample_supply =
            create_typed_buffer(rc, Nb * S, sizeof(float), "sample_supply");
        ...
        auto cmd = rc.create(CommandListDesc{});
        cmd->open();
        std::vector<float> zeros_bristle(
            Nb * M * (PaintSimStorage::BRISTLE_VERTEX_STRIDE / sizeof(float)),
            0.0f);
        cmd->writeBuffer(
            storage.bristle_data,
            zeros_bristle.data(),
        ...
        cmd->writeBuffer(
            ...
            Nb * S * sizeof(float) * 4);
        cmd->writeBuffer(
            storage.sample_liquid_b,
            zeros_sample.data(),
            Nb * S * sizeof(float) * 4);

        // Set user paint color (from stroke color, default RYB red)
        glm::vec3 ink_color =
            (colors.size() > 0) ? colors.back() : glm::vec3(1.0f, 0.0f, 0.0f);
        float input_color[4] = {
            ink_color.r, ink_color.g, ink_color.b, ink_amount
        };
        cmd->writeBuffer(
            storage.bristle_input_color_buf, input_color, sizeof(float) * 4);
    ...
    if (!storage.particles_initialized) {
        ...
        safe_destroy_buf(storage.ptcl_pos);
        safe_destroy_buf(storage.ptcl_vel);
        safe_destroy_buf(storage.ptcl_color);
        safe_destroy_buf(storage.ptcl_alive);
        safe_destroy_buf(storage.ptcl_counter);
        safe_destroy_buf(storage.ptcl_pos_b);
        safe_destroy_buf(storage.ptcl_vel_b);
        safe_destroy_buf(storage.ptcl_color_b);
        safe_destroy_buf(storage.ptcl_alive_b);

        storage.ptcl_pos =
            create_typed_buffer(rc, max_ptcl, sizeof(float) * 4, "ptcl_pos");
        storage.ptcl_vel =
            create_typed_buffer(rc, max_ptcl, sizeof(float) * 4, "ptcl_vel");
        storage.ptcl_color =
            create_typed_buffer(rc, max_ptcl, sizeof(float) * 4, "ptcl_color");
        storage.ptcl_alive =
            create_typed_buffer(rc, max_ptcl, sizeof(uint32_t), "ptcl_alive");
        storage.ptcl_counter =
            create_byte_buffer(rc, sizeof(uint32_t), "ptcl_counter");
        storage.ptcl_pos_b =
            create_typed_buffer(rc, max_ptcl, sizeof(float) * 4, "ptcl_pos_b");
        storage.ptcl_vel_b =
            create_typed_buffer(rc, max_ptcl, sizeof(float) * 4, "ptcl_vel_b");
        storage.ptcl_color_b = create_typed_buffer(
            rc, max_ptcl, sizeof(float) * 4, "ptcl_color_b");
        storage.ptcl_alive_b =
            create_typed_buffer(rc, max_ptcl, sizeof(uint32_t), "ptcl_alive_b");

        auto cmd = rc.create(CommandListDesc{});
        cmd->open();
        std::vector<float> zeros_ptcl(max_ptcl * 4, 0.0f);
        cmd->writeBuffer(
            storage.ptcl_pos, zeros_ptcl.data(), max_ptcl * sizeof(float) * 3);
        cmd->writeBuffer(
            storage.ptcl_vel, zeros_ptcl.data(), max_ptcl * sizeof(float) * 3);
        cmd->writeBuffer(
            storage.ptcl_color,
            zeros_ptcl.data(),
            max_ptcl * sizeof(float) * 4);
        std::vector<uint32_t> zeros_u(max_ptcl, 0);
        cmd->writeBuffer(
            storage.ptcl_alive, zeros_u.data(), max_ptcl * sizeof(uint32_t));
        uint32_t zero_c = 0;
        cmd->writeBuffer(storage.ptcl_counter, &zero_c, sizeof(uint32_t));
        cmd->writeBuffer(
            storage.ptcl_pos_b,
            zeros_ptcl.data(),
            max_ptcl * sizeof(float) * 3);
        cmd->writeBuffer(
            storage.ptcl_vel_b,
            zeros_ptcl.data(),
            max_ptcl * sizeof(float) * 3);
        cmd->writeBuffer(
            storage.ptcl_color_b,
            zeros_ptcl.data(),
            max_ptcl * sizeof(float) * 4);
        cmd->writeBuffer(
            storage.ptcl_alive_b, zeros_u.data(), max_ptcl * sizeof(uint32_t));
        cmd->close();
        device->executeCommandList(cmd);
        device->waitForIdle();
        rc.destroy(cmd);
    ...
    if (!storage.divergence_program)
        storage.divergence_program =
            compile_shader(rc, "fluid_divergence.slang");
    ...
    if (!storage.bristle_sim_program)
        storage.bristle_sim_program =
            compile_shader(rc, "bristle_simulate.slang");
    if (!storage.bristle_density_constraint_program)
        storage.bristle_density_constraint_program =
            compile_shader(rc, "bristle_density_constraint.slang");
    if (!storage.bristle_resample_program)
        storage.bristle_resample_program =
            compile_shader(rc, "bristle_resample.slang");
    if (!storage.bristle_raster_program)
        storage.bristle_raster_program =
            compile_shader(rc, "bristle_rasterize.slang");
    if (!storage.bristle_merge_program)
        storage.bristle_merge_program =
            compile_shader(rc, "bristle_merge.slang");
    ...
    if (!storage.ptcl_update_program)
        storage.ptcl_update_program =
            compile_shader(rc, "particle_update.slang");
    if (!storage.ptcl_raster_program)
        storage.ptcl_raster_program =
            compile_shader(rc, "particle_rasterize.slang");
    if (!storage.ptcl_flip_pic_program)
        storage.ptcl_flip_pic_program =
            compile_shader(rc, "particle_flip_pic.slang");
    if (!storage.ptcl_compact_program)
        storage.ptcl_compact_program =
            compile_shader(rc, "particle_compact.slang");
    if (!storage.ptcl_to_grid_program)
        storage.ptcl_to_grid_program =
            compile_shader(rc, "particle_to_grid.slang");
    if (!storage.grid_to_ptcl_program)
        storage.grid_to_ptcl_program =
            compile_shader(rc, "grid_to_particle.slang");

    // Bristle liquid transfer shaders (Section 5.1)
    if (!storage.bri_liquid_transfer_program)
        storage.bri_liquid_transfer_program =
            compile_shader(rc, "bristle_liquid_transfer.slang");
    if (!storage.bri_liquid_emit_program)
        storage.bri_liquid_emit_program =
            compile_shader(rc, "bristle_liquid_emit.slang");

    // Canvas commit shader (2D canvas layer persistence)
    if (!storage.canvas_commit_program)
        storage.canvas_commit_program =
            compile_shader(rc, "canvas_commit.slang");
    ...
    if (!storage.deposit_program || !storage.advect_program ||
        ...
        !storage.bri_liquid_transfer_program ||
        !storage.bri_liquid_emit_program || !storage.ptcl_emit_program ||
        !storage.ptcl_update_program || !storage.ptcl_raster_program ||
        !storage.ptcl_flip_pic_program || !storage.ptcl_compact_program ||
        !storage.ptcl_to_grid_program || !storage.grid_to_ptcl_program) {
    ...
    // in the shader (spread_xy = max(1, ceil(r/cell))), not in the world
    // radius.
    ...
    if (!vertices.empty()) {
        ...
        if (!new_stroke && last > 0) {
            ...
            if (last > 1 && !vertex_is_stroke_start(last - 1)) {
                ...
                brush_angular_accel =
                    (new_omega - storage.prev_angular_vel) / frame_dt;
    ...
    // so that the loop can compute frame_disp = ||brush_pos_3d - prev||.
    // Setting it here (before the loop) would make every frame's delta zero.
    ...
    const int WIN_XY =
        std::min(PaintSimStorage::WIN_ALLOC_XY, storage.grid_res);
    const int WIN_Z = rz;
    ...
    auto position_window = [&](float bx, float by) {
        ...
        if (moved && storage.win_origin_set) {
            ...
            cctx.begin();
            cctx.dispatch(
                {}, cv, WIN_XY * WIN_XY, 256);  // one thread per column
            cctx.finish();
            rc.destroy(commit_cb_buf);

            // Clear the 3D window fields so the new region starts empty.
            // GPU compute clears (field_clear.slang: one thread per float)
            // instead of the former CPU-allocate + 10× writeBuffer upload (~21
            // MB/frame). Same buffers, same zero value, same total thread count
            // win_n3d as the loop above. The caller-visible contract is
            // unchanged: on return the 10 live window buffers are zeroed and
            // GPU work is flushed.
            for (auto* b : { &storage.density,
                             &storage.color_r,
                             &storage.color_y,
                             &storage.color_b,
                             &storage.wetness,
    ...
    auto deposit_at = [&](const glm::vec3& sub_pos,
        ...
        upload_constant_buffer(
            rc, device, &mc, sizeof(SimConstants), "merge_cb", merge_cb);
    ...
    // live window (the geometric hole that previously caused 断续 is gone).
    {
        ...
        spdlog::info(
            "brush_paint_sim sub-step: n_sub={}, frame_disp={:.4f} "
        ...
        // (frame_disp = ||next_brush_pos - this_brush_pos||). Must run after
        // the loop so the loop sees the previous frame's position.
        if (!vertices.empty()) {
        ...
        if (std::getenv("RUZINO_TRACE_BRISTLES") != nullptr) {
            ...
            cmd->open();
            cmd->copyBuffer(
                rb, 0, storage.sample_pos, 0, n_samples * sizeof(float) * 4);
            cmd->close();
            device->executeCommandList(cmd);
            device->waitForIdle();
            ...
            rc.destroy(cmd);
            spdlog::info(
                "brush_paint_sim bristle-trace: brush=({:.3f},{:.3f}) "
    ...
    if (storage.particles_initialized && new_count > 0 && !DISABLE_PARTICLES) {
        ...
        upload_constant_buffer(
            rc, device, &pc, sizeof(ParticleConstants), "ptcl_cb", ptcl_cb);
        ...
        upload_constant_buffer(
            rc, device, &pc, sizeof(ParticleConstants), "emit0_cb", emit0_cb);
        dispatch_field(
            rc,
            storage.ptcl_emit_program,
            { { "sample_pos", storage.sample_pos },
              { "sample_color", storage.sample_color },
              { "density", storage.density } },
            { { "ptcl_counter", storage.ptcl_counter },
        ...
        // Bind all declared SRVs even if mode 1 doesn't use
        // sample_pos/sample_color
        pc.emit_mode = 1;
        nvrhi::BufferHandle emit1_cb;
        upload_constant_buffer(
            rc, device, &pc, sizeof(ParticleConstants), "emit1_cb", emit1_cb);
        dispatch_field(
            rc,
            storage.ptcl_emit_program,
            { { "sample_pos", storage.sample_pos },
        ...
        upload_constant_buffer(
            rc, device, &mc2, sizeof(SimConstants), "ptcl_merge_cb", merge_cb);
    ...
    */
    ...
    if (sim_dt > 1e-6f) {  // TEMP reverted: fluid re-enabled
        ...
        for (int s = 0; s < substeps; s++) {
                ...
                static_cast<float>(storage.grid_res * storage.grid_res);
            {
                ...
                dispatch_field(
                    rc,
                    storage.jacobi_program,
                    { { "field_in", storage.vel_x },
                      { "rhs", storage.vel_x },
                      { "bristle_psi", storage.bristle_density },
                      { "wetness", storage.wetness } },
                    { { "field_out", storage.vel_x_tmp } },
                    jcb,
                    window_total);
            ...
            for (int fp = 0; fp < 3; fp++) {
                dispatch_field(
                    ...
                    { { "vel_x", storage.vel_x },
                      ...
                      { "bristle_psi", storage.bristle_density },
                      { "wetness", storage.wetness } },
                    { { "div_out", storage.divergence_buf } },
                    cb_buf,
                    window_total);
            ...
            dispatch_field(
                ...
                { { "field_in", storage.oil_density },
                  ...
                  { "vel_y", storage.vel_y },
                  { "vel_z", storage.vel_z } },
                { { "field_out", storage.oil_density_tmp } },
                cb_buf,
                window_total);
    ...
    if (storage.particles_initialized && !DISABLE_PARTICLES) {
        ...
        upload_constant_buffer(
            rc, device, &pc, sizeof(ParticleConstants), "maint_cb", maint_cb);
        ...
        // Grid to particle (emit near brush, with stratified sampling + Eq.15
        // density subtraction) Uses density_tmp as output to avoid read-write
        // hazard, then swap
        dispatch_field(
            rc,
            storage.grid_to_ptcl_program,
            ...
            { { "ptcl_counter", storage.ptcl_counter },
              ...
              { "ptcl_alive", storage.ptcl_alive },
              { "density_out",
                storage.density_tmp } },  // Eq.15: density reduced to tmp
            maint_cb,
            win_n3d);
        std::swap(storage.density, storage.density_tmp);
        ...
        for (int dz = 0; dz < WIN_Z; ++dz)
            for (int dy = 0; dy < WIN_XY; ++dy)
                for (int dx = 0; dx < WIN_XY; ++dx) {
                    // Window-local linear index (matches grid_idx_3d with WIN
                    // strides).
                    int gi = dz * WIN_XY * WIN_XY + dy * WIN_XY + dx;
                    if (gi < 0 || gi >= readback_n)
                        continue;
    ...
    float mean_div =
        div_count > 0
            ? static_cast<float>(div_sum / static_cast<double>(div_count))
            : 0.0f;
        ...
        if (ptcl_count > 0 && storage.ptcl_color && storage.ptcl_alive) {
            ...
            auto read_structured =
                [&](nvrhi::BufferHandle buf, int elem_bytes, void* dst) {
                    ...
                    cmd->open();
                    cmd->copyBuffer(
                        rb, 0, buf, 0, static_cast<size_t>(n) * elem_bytes);
                    cmd->close();
                    device->executeCommandList(cmd);
                    device->waitForIdle();
                    void* mapped =
            ...
            read_structured(
                storage.ptcl_color, sizeof(float) * STRIDE, cols.data());
    ...
    for (int y = 0; y < storage.grid_res; ++y) {
        for (int x = 0; x < storage.grid_res; ++x) {
            ...
            if (cdensity[gi] <= threshold)
                continue;
            // canvas_color stores density-weighted RYB sum; normalize by
            // density to recover the average pigment.
            float d = cdensity[gi];
    ...
    // problem (density present but too small to render visibly).
    {
        ...
        spdlog::info(
            ...
            d_above_thresh,
            d_above_quarter,
            d_count > 0 ? d_min : 0.0f,
            d_max,
            d_count > 0 ? d_sum / d_count : 0.0f);
    }

    spdlog::info(
        "brush_paint_sim: {} new verts, {} substeps, {} particles",
        new_count,
        sim_dt > 1e-6f ? std::min(
                             16,
                             std::max(
                                 1,
                                 static_cast<int>(std::ceil(
                                     sim_dt / (2.0f / storage.grid_res)))))
                       : 0,
        out_pts.size());
