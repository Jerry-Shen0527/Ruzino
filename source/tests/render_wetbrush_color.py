...
def build_sim_graph(sim_usd, ryb, res, res_z, paper):
    ...
    g.setSocketDefaults({
        # Speed 0.3: the 1s stroke (Length 0.3) completes within the 60-frame
        # (1s) sequence — same pacing as the old 30-point/30fps curve.
        (pen, "Length"): 0.3, (pen, "Amplitude"): 0.02,
        (pen, "Speed"): 0.3,
        (pen, "Ink R (RYB)"): float(ryb[0]),
        (pen, "Ink Y (RYB)"): float(ryb[1]),
        (pen, "Ink B (RYB)"): float(ryb[2]),
        (sim, "Resolution"): res, (sim, "Resolution Z"): res_z,
        (sim, "Paper Size"): paper,
